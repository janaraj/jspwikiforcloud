...
/**
 * <p>
 * Defines Commands for viewing, editing and deleting wiki groups. GroupCommands
 * can be combined with GroupPrincipals to produce targeted Commands.
 * </p>
 * <p>
...
public final class GroupCommand extends AbstractCommand {
	...
	public static final Command DELETE_GROUP = new GroupCommand("deleteGroup",
			"%uDeleteGroup.jsp?group=%n", null, new GroupPrincipal(
	...
	public static final Command EDIT_GROUP = new GroupCommand("editGroup",
			"%uEditGroup.jsp?group=%n", "EditGroupContent.jsp",
			new GroupPrincipal(GroupPermission.EDIT_ACTION),
	...
	public static final Command VIEW_GROUP = new GroupCommand("viewGroup",
			"%uGroup.jsp?group=%n", "GroupContent.jsp", new GroupPrincipal(
					GroupPermission.VIEW_ACTION), GroupPermission.VIEW_ACTION);
	 ...
	 *            the URL pattern
	 * @param target
	 *            the target of this command (a GroupPrincipal representing a
	 *            Group); may be <code>null</code>
	 * @param action
	 *            the action used to construct a suitable GroupPermission
	 ...
	 *             <code>null</code>
	 */
	private GroupCommand(String requestContext, String urlPattern,
			String contentTemplate, GroupPrincipal target, String action) {
	...
	/**
	 * Creates and returns a targeted Command by combining a GroupPrincipal with
	 * this Command. The supplied <code>target</code> object must be non-
	 * <code>null</code> and of type GroupPrincipal. If the target is not of the
	 * correct type, this method throws an {@link IllegalArgumentException}.
	 * 
	 * @param target
	...
	public final Command targetedCommand(Object target) {
		if (!(target != null && target instanceof GroupPrincipal)) {
			throw new IllegalArgumentException(
					"Target must non-null and of type GroupPrincipal.");
		}
		return new GroupCommand(getRequestContext(), getURLPattern(),
				getContentTemplate(), (GroupPrincipal) target, m_action);
	}
	...
	public final Permission requiredPermission() {
		...
		GroupPrincipal target = (GroupPrincipal) m_target;
		if (target == null || m_action == null) {
			m_permission = null;
		} else {
			m_permission = new GroupPermission(target.getName(),
					m_action);
		}
		return m_permission;
	}
