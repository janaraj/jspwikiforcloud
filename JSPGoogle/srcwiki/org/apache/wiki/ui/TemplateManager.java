...
public class TemplateManager extends ModuleManager {
	 ...
	 *            the JSP PageContext
	 * @param name
	 *            The name of the JSP page to look for (e.g "Wiki.jsp")
	 ...
	 *            The JSP PageContext
	 * @param template
	 *            From which template we should seek initially?
	 * @param name
	...
	public String findJSP(PageContext pageContext, String template, String name) {
		if (name == null || template == null) {
			log.fatal("findJSP() was asked to find a null template or name ("
					+ template
					+ ","
					+ name
					+ ")."
			...
			throw new InternalWikiException(
					"Illegal arguments to findJSP(); please check logs.");
		}
	 ...
	 * @param name
	 *            the name of the resource to fine
	 ...
	 * 
	 * try { Properties props = (Properties)m_propertyCache.getFromCache(
	 * template, -1 );
	 * 
	 * if( props == null ) { try { props = getTemplateProperties( template );
	 * 
	 ...
	 * return null; } }
	 * 
	 * return props.getProperty( key ); } catch( NeedsRefreshException ex ) { //
	 * FIXME return null; } }
	 */
	/**
	...
	public Map listLanguages(PageContext pageContext) {
		if (false) {
			return listLanguagesFromJar(pageContext);
		} else {
			return listLanguagesFromDirectory(pageContext);
		}
	}
	...
	private void putLocaleName(PageContext pageContext, Map resultMap,
		...
		if (clientLanguage.startsWith(name)) {
			defaultLanguage = LocaleSupport.getLocalizedMessage(pageContext,
					I18NDEFAULT_LOCALE);
		}
	...
	private Map listLanguagesFromDirectory(PageContext pageContext) {
	...
	private Map listLanguagesFromJar(PageContext pageContext) {
		LinkedHashMap<String, String> resultMap = new LinkedHashMap<String, String>();
		...
		try {
			...
			InputStream inputStream = pageContext.getServletContext()
					.getResourceAsStream(I18NRESOURCE_PATH);
	...
	public Map listTimeZones(PageContext pageContext) {
		...
		String[][] tzs = {
				...
				{ "GMT",
						"Casablanca, Dublin, Edinburgh, London, Lisbon, Monrovia" },
				{ "GMT+1", "Berlin, Brussels, Copenhagen, Madrid, Paris, Rome" },
				{ "GMT+2",
						"Helsinki, Athens, Kaliningrad, South Africa, Warsaw" },
		...
		for (int i = 0; i < tzs.length; i++) {
			...
			if (servertz.getRawOffset() == tz.getRawOffset()) {
				serverTimeZone = LocaleSupport.getLocalizedMessage(pageContext,
						I18NSERVER_TIMEZONE);
				tzID = servertz.getID();
			}
	 ...
	 * 
	 * if( propertyStream != null ) { p.load( propertyStream );
	 * 
	 * propertyStream.close(); } else {
	 * log.debug("Template '"+templateName+"' does not have a propertyfile '"
	 * +PROPERTYFILE+"'."); }
	 * 
	 * return p; }
	 */
	 ...
	 * @param type
	 *            What kind of a request should be added?
	 * @param resource
	 *            The resource to add.
	 */
	@SuppressWarnings("unchecked")
	public static void addResourceRequest(WikiContext ctx, String type,
		...
		if (type.equals(RESOURCE_SCRIPT)) {
			resourceString = "<script type='text/javascript' src='" + resource
					+ "'></script>";
		} else if (type.equals(RESOURCE_STYLESHEET)) {
			resourceString = "<link rel='stylesheet' type='text/css' href='"
					+ resource + "' />";
	...
	public static String[] getResourceRequests(WikiContext ctx, String type) {
		HashMap<String, Vector<String>> hm = (HashMap<String, Vector<String>>) ctx
				.getVariable(RESOURCE_INCLUDES);
	...
	public static String[] getResourceTypes(WikiContext ctx) {
		...
		if (ctx != null) {
			HashMap<String, String> hm = (HashMap<String, String>) ctx
					.getVariable(RESOURCE_INCLUDES);
