...
/**
 * Filter that verifies that the {@link org.apache.wiki.WikiEngine} is running,
 * and sets the authentication status for the user's WikiSession. Each HTTP
 * request processed by this filter is wrapped by a {@link WikiRequestWrapper}.
 * The wrapper's primary responsibility is to return the correct
 * <code>userPrincipal</code> and <code>remoteUser</code> for authenticated
 * JSPWiki users (whether authenticated by container or by JSPWiki's custom
 * system). The wrapper's other responsibility is to incorporate JSPWiki
 * built-in roles into the role-checking algorithm for
...
public class WikiServletFilter implements Filter {
	 ...
	 * executes, which sets the authentication state. Then, the request is
	 * wrapped with a {@link WikiRequestWrapper}.
	 * 
	 * @param request
	 ...
	 * @throws ServletException
	 *             if {@link AuthenticationManager#login(HttpServletRequest)}
	...
	/**
	 * Figures out the wiki context from the request. This method does not
