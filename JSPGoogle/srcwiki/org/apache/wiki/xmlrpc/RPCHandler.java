...
public class RPCHandler extends AbstractRPCHandler {
    ...
    public String getApplicationName() {
        ...
        return toRPCString(BeanHolder.getApplicationName());
    }
    ...
    protected Hashtable<String, Object> encodeWikiPage(WikiPage page) {
        ...
        cal.setTime(d);
        cal.add(Calendar.MILLISECOND, -(cal.get(Calendar.ZONE_OFFSET) + (cal
                .getTimeZone().inDaylightTime(d) ? cal.get(Calendar.DST_OFFSET)
                : 0)));
    ...
    public Vector getRecentChanges(Date since) {
        ...
        cal.add(Calendar.MILLISECOND,
                (cal.get(Calendar.ZONE_OFFSET) + (cal.getTimeZone()
                        .inDaylightTime(since) ? cal.get(Calendar.DST_OFFSET)
                        : 0)));
        since = cal.getTime();
    ...
    private String parsePageCheckCondition(String pagename)
        ...
        if (!m_engine.pageExists(pagename)) {
            throw new XmlRpcException(ERR_NOPAGE, "No such page '" + pagename
                    + "' found, o master.");
        }
    ...
    public Vector listLinks(String pagename) throws XmlRpcException {
        ...
        for (Iterator i = localCollector.getLinks().iterator(); i.hasNext();) {
            ...
            // from the TranslatorReader itself. Also, the link format should
            // probably
