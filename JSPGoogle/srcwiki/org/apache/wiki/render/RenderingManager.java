...
/**
 * This class provides a facade towards the differing rendering routines. You
 * should use the routines in this manager instead of the ones in WikiEngine, if
 * you don't want the different side effects to occur - such as WikiFilters.
 * <p>
 ...
 * @since 2.4
 */
public class RenderingManager extends AbstractWikiProvider implements
        WikiEventListener, InternalModule {
    private static Log log = LogFactory.getLog(RenderingManager.class);

    private int m_cacheExpiryPeriod = 24 * 60 * 60; // This can be relatively
                                                    // long
     ...
     * Boolean.FALSE depending on whether WYSIWYG is currently in effect.
     */
     ...
     * Value can be either Boolean.TRUE or Boolean.FALSE.
     */
    public static final String VAR_EXECUTE_PLUGINS = "_PluginContent.execute";

    private String renderImplName;

    private Constructor getC() {
        Class[] rendererParams = { WikiContext.class, WikiDocument.class };
        Constructor co = null;
        try {
            Class c = Class.forName(renderImplName);
            co = c.getConstructor(rendererParams);
        } catch (ClassNotFoundException e) {
            log.error("Unable to find WikiRenderer implementation "
                    + renderImplName);
        } catch (SecurityException e) {
            log.error("Unable to access the WikiRenderer(WikiContext,WikiDocument) constructor for "
                    + renderImplName);
        } catch (NoSuchMethodException e) {
            log.error("Unable to locate the WikiRenderer(WikiContext,WikiDocument) constructor for "
                    + renderImplName);
        }
        // if( co == null )
        // {
        // throw new WikiException( "Failed to get WikiRenderer '" +
        // renderImplName + "'." );
        // }
    ...
    public RenderingManager(WikiEngine engine) throws WikiException {
        initialize(engine);
    }

    /**
     * Initializes the RenderingManager. Checks for cache size settings,
     * initializes the document cache. Looks for alternative WikiRenderers,
     * initializes one, or the default XHTMLRenderer, for use.
     * 
     * @param engine
     ...
     *             If the manager could not be initialized.
     */
    public void initRendering(FilterManager fManager) throws WikiException {
        int cacheSize = TextUtil.getIntegerProperty(m_properties,
                PROP_CACHESIZE, -1);

        if (cacheSize == -1) {
            cacheSize = TextUtil.getIntegerProperty(m_properties,
                    CachingProvider.PROP_CACHECAPACITY, DEFAULT_CACHESIZE);
        }

        if (cacheSize > 0) {
            m_documentCache = new Cache(true, false, false, false,
                    OSCACHE_ALGORITHM, cacheSize);
        } else {
            log.info("RenderingManager caching is disabled.");
        }

        renderImplName = m_properties.getProperty(PROP_RENDERER);
        ...
        // {
        // log.error(
        // "Unable to access the WikiRenderer(WikiContext,WikiDocument) constructor for "
        // + renderImplName );
        // }
        // catch( NoSuchMethodException e )
        // {
        // log.error(
        // "Unable to locate the WikiRenderer(WikiContext,WikiDocument) constructor for "
        // + renderImplName );
        // }
        ...
        WikiEventUtils.addWikiEventListener(m_engine,
                WikiPageEvent.POST_SAVE_BEGIN, this, fManager);
    }
     ...
     * @throws IOException
     *             If rendering cannot be accomplished
     */
    ...
    protected WikiDocument getRenderedDocument(WikiContext context,
            String pagedata) throws IOException {
        ...
        if (m_documentCache != null) {
            try {
                WikiDocument doc = (WikiDocument) m_documentCache.getFromCache(
                        pageid, m_cacheExpiryPeriod);
    ...
    /**
     * Simply renders a WikiDocument to a String. This version does not get the
     * document from the cache - in fact, it does not cache the document at all.
     * This is very useful, if you have something that you want to render
     * outside the caching routines. Because the cache is based on full pages,
     * and the cache keys are based on names, use this routine if you're
     * rendering anything for yourself.
     * 
     * @param context
     *            The WikiContext to render in
     * @param doc
    ...
    public WikiRenderer getRenderer(WikiContext context, WikiDocument doc) {
        ...
        try {
            rval = (WikiRenderer) getC().newInstance(params);
        } catch (Exception e) {
     ...
     * Note that you can't use this method to do any arbitrary rendering, as the
     * pagedata MUST be the data from the that the WikiContext refers to - this
     * method caches the HTML internally, and will return the cached version. If
     * the pagedata is different from what was cached, will re-render and store
     * the pagedata into the internal cache.
     * 
     * @param context
     *            the wiki context
     * @param pagedata
    ...
    public void actionPerformed(WikiEvent event) {
        if ((event instanceof WikiPageEvent)
            ...
            if (m_documentCache != null) {
                ...
                Collection referringPages = BeanHolder.getReferenceManager()
                        .findReferrers(pageName);
