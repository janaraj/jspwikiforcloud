 ...
 * @since 2.2.21.
 */
public class LuceneSearchProvider extends AbstractWikiProvider implements SearchProvider {
	...
	public static final String[] SEARCHABLE_FILE_SUFFIXES = new String[] {
			...
			".java", ".c", ".cpp", ".php", ".asm", ".sh", ".properties",
			".kml", ".gpx", ".loc" };
	...
	public void initialize(WikiEngine engine, Properties props)
		...
		int initialDelay = TextUtil.getIntegerProperty(props,
				PROP_LUCENE_INITIALDELAY, LuceneUpdater.INITIAL_DELAY);
		int indexDelay = TextUtil.getIntegerProperty(props,
				PROP_LUCENE_INDEXDELAY, LuceneUpdater.INDEX_DELAY);

		m_analyzerClass = TextUtil.getStringProperty(props,
				PROP_LUCENE_ANALYZER, m_analyzerClass);
		...
		try {
			...
			if (!dir.exists() || !dir.canWrite() || !dir.canRead()) {
				log.error("Cannot write to Lucene directory, disabling Lucene: "
						+ dir.getAbsolutePath());
			...
			if (filelist == null) {
				throw new IOException(
						"Invalid Lucene directory: cannot produce listing: "
								+ dir.getAbsolutePath());
			}
		} catch (IOException e) {
			log.error(
					"Problem while creating Lucene index - not using Lucene.",
					e);
		}
		...
		LuceneUpdater updater = new LuceneUpdater(m_engine, this, initialDelay,
				indexDelay);
		updater.start();
	}
	...
	protected void doFullLuceneReindex() throws IOException {
		...
		try {
			...
			log.error(
					"Problem reading pages while creating Lucene index (JSPWiki won't start.)",
					e);
	...
	protected synchronized void updateLuceneIndex(WikiPage page, String text) {
		...
		try {
			...
			log.error("Unable to update page '" + page.getName()
					+ "' from Lucene index", e);
		} catch (Exception e) {
			log.error(
					"Unexpected Lucene exception - please check configuration!",
					e);
		} finally {
			try {
				if (writer != null)
	...
	private Analyzer getLuceneAnalyzer() throws ClassNotFoundException,
	 ...
	 */
	protected Document luceneIndexPage(WikiPage page, String text,
		...
		field = new Field(LUCENE_PAGE_CONTENTS, text, Field.Store.YES,
		...
		String unTokenizedTitle = StringUtils.replaceChars(page.getName(),
				MarkupParser.PUNCTUATION_CHARS_ALLOWED, c_punctuationSpaces);

		field = new Field(LUCENE_PAGE_NAME, TextUtil.beautifyString(page
				.getName()) + " " + unTokenizedTitle, Field.Store.YES,
				Field.Index.TOKENIZED, Field.TermVector.NO);
		doc.add(field);
		...
		if (page.getAuthor() != null) {
			field = new Field(LUCENE_AUTHOR, page.getAuthor(), Field.Store.YES,
					Field.Index.TOKENIZED, Field.TermVector.NO);
			doc.add(field);
		}
	...
	public void reindexPage(WikiPage page) {
		if (page != null) {
			...
			if (text != null) {
				...
				pair[1] = text;
				m_updates.add(pair);
				log.debug("Scheduling page " + page.getName()
						+ " for index update");
			}
		}
	}
	...
	public Collection findPages(String query, int flags)
		...
		try {
				...
				if (page != null) {
					...
					if (text != null && highlighter != null) {
						TokenStream tokenStream = getLuceneAnalyzer()
								.tokenStream(LUCENE_PAGE_CONTENTS,
					...
					log.error("Lucene found a result page '"
							+ pageName
			...
			throw new ProviderException(
					"You have entered a query Lucene cannot process: "
							+ e.getMessage());
		} finally {
			if (searcher != null) {
				try {
