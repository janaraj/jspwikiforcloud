...
public class FormOutput extends FormElement {
    /**
     * Executes the FormHandler specified in a Form 'output' plugin, using
     * entries provided in the HttpRequest as FormHandler parameters.
     * <p>
     * If the parameter 'populate' was given, the WikiPlugin it names is used to
     * get default values. (It probably makes a lot of sense for this to be the
     * same plugin as the handler.) Information for the populator can be given
     * with the FormSet plugin. If 'populate' is not specified, the form is not
     * displayed.
     * <p>
     * Should there be no HTTP request associated with this request, the method
    ...
    public String execute(WikiContext ctx, Map params) throws PluginException {
        ...
        if (handler == null || handler.length() == 0) {
            ...
            return "<p class=\"error\">"
                    + MessageFormat.format(
                            rb.getString("formoutput.missingargument"), args)
                    + "</p>";
        }
        ...
        try {
            ...
            handlerOutput = pManager
                    .execute(ctx, handler, info.getSubmission());
