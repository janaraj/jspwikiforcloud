 ...
 * @since 2.2
 */
public class DefaultURLConstructor extends AbstractWikiProvider implements
        URLConstructor {
    ...
    public DefaultURLConstructor(WikiEngine engine) throws WikiException {
        initialize(engine);
    }
    ...
    public void initialize(WikiEngine engine, Properties properties) {
        ...
        m_useRelativeURLStyle = "relative".equals(properties.getProperty(
                WikiEngine.PROP_REFSTYLE, "relative"));
     ...
     * 
     * <ul>
     * <li>"%u" - inserts either the base URL (when absolute is required), or
     * the base path (which is an absolute path without the host name).
    ...
    protected final String doReplacement(String baseptrn, String name,
        ...
        baseptrn = TextUtil
                .replaceString(baseptrn, "%U", m_engine.getBaseURL());
     ...
     * request.getPathInfo() will return faulty results for paths which contains
     * + signs to signify spaces.
     * <p>
     * This method provides a workaround by simply parsing the getRequestURI(),
     * which is returned from the servlet container undedecoded.
     * <p>
     * Please see <a
     * href="http://issues.apache.org/bugzilla/show_bug.cgi?id=39278">Tomcat Bug
     * 39278</a> for more information.
     * 
     * @param request
     *            A HTTP servlet request
     * @param encoding
     *            The used encoding
     * @return a String, decoded by JSPWiki, specifying extra path information
     *         that comes after the servlet path but before the query string in
     *         the request URL; or null if the URL does not have any extra path
     *         information
     * @throws UnsupportedEncodingException
     */
    /*
     * private static String getPathInfo( HttpServletRequest request, String
     * encoding ) throws UnsupportedEncodingException { String c =
     * request.getContextPath(); // Undecoded String s =
     * request.getServletPath(); // Decoded String u = request.getRequestURI();
     * // Undecoded
     * 
     * c = URLDecoder.decode( c, encoding ); u = URLDecoder.decode( u, encoding
     * );
     * 
