...
public class ContextualDiffProvider extends AbstractWikiProvider implements DiffProvider {

	private static final Log log = LogFactory
			.getLog(ContextualDiffProvider.class);

	/**
	 * A jspwiki.properties value to define how many characters are shown around
	...
	public void initialize(WikiEngine engine, Properties properties)
		...
		String configuredLimit = properties
				.getProperty(PROP_UNCHANGED_CONTEXT_LIMIT,
						Integer.toString(LIMIT_MAX_VALUE));
		int limit = LIMIT_MAX_VALUE;
		try {
			limit = Integer.parseInt(configuredLimit);
		} catch (NumberFormatException e) {
			log.warn("Failed to parseInt " + PROP_UNCHANGED_CONTEXT_LIMIT + "="
					+ configuredLimit + "   Will use a huge number as limit.",
					e);
		}
	 ...
	 * 
	 * @see org.apache.wiki.diff.DiffProvider#makeDiffHtml(WikiContext, String,
	 *      String)
	 * 
	 *      {@inheritDoc}
	 */
	...
	private String[] sequence(String wikiText) {
		...
		for (int i = 0; i < linesArray.length; i++) {
			...
			// here...
			for (StringTokenizer st = new StringTokenizer(line, " ", true); st
					.hasMoreTokens();) {
				token = st.nextToken();
	...
	private final class ChangeMerger implements RevisionVisitor {
		...
		private ChangeMerger(final StringBuffer sb, final String[] origStrings,
				final int max) {
			m_sb = sb;
		...
		private void updateState(Delta delta) {
			...
			if (orig.first() > m_firstElem) {
				...
				if ((orig.first() - m_firstElem) > 2 * m_unchangedContextLimit) {
					if (m_firstElem > 0) {
						int endIndex = Math.min(m_firstElem
								+ m_unchangedContextLimit,
								m_origStrings.length - 1);
					...
					int startIndex = Math.max(orig.first()
		...
		public void shutdown() {
			...
			if (m_firstElem < m_origStrings.length) {
				...
				if ((m_origStrings.length - m_firstElem) > m_unchangedContextLimit) {
					int endIndex = Math
							.min(m_firstElem + m_unchangedContextLimit,
									m_origStrings.length - 1);
