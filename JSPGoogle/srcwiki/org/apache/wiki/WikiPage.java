...
/**
 * Simple wrapper class for the Wiki page attributes. The Wiki page content is
...
public class WikiPage implements Cloneable, Comparable {
     ...
     * with the wiki page to the page repository, but are generated on-the-fly.
     * A provider is not required to save them, but they can do that if they
     * really want.
     * 
     * @param key
     ...
     * @param key
     *            The key for the attribute used to fetch the attribute later
     *            on.
     * @param attribute
     *            The attribute value
     */
    ...
    /**
     * Sets the last modification date. In general, this is only changed by the
     * provider.
     * 
     * @param date
     *            The date
     ...
     * there is no Acl defined, or it has not yet been set by
     * {@link #setAcl(Acl)}.
     * 
     ...
     * the Acl itself to back-end storage or in page markup; it merely sets the
     * internal field that stores the Acl. To persist the Acl, callers should
     * invoke
    ...
    /**
     * Returns <code>true</code> if the page has valid metadata; that is, it has
     * been parsed. Note that this method is a kludge to support our pre-3.0
     * metadata system, and as such will go away with the new API.
     * 
    ...
    public String toString() {
        return "WikiPage [" + m_wiki + ":" + m_name + ",ver=" + m_version
                + ",mod=" + m_lastModified + "]";
    }

    /**
     * Creates a deep clone of a WikiPage. Strings are not cloned, since they're
     * immutable. Attributes are not cloned, only the internal HashMap (so if
     * you modify the contents of a value of an attribute, these will reflect
     * back to everyone).
     * 
    ...
    public Object clone() {
        ...
        p.m_lastModified = m_lastModified != null ? (Date) m_lastModified
                .clone() : null;
    ...
    public int compareTo(Object page) {
        PageSorter sorter = BeanHolder.getPageSorter();
        return sorter.compare(this, (WikiPage) page);
    }
     ...
     * WikiPage ) { WikiPage oo = (WikiPage) o;
     * 
     * if( oo.getName().equals( getName() ) ) { if( oo.getVersion() ==
