...
public class IfPlugin implements WikiPlugin {
	 ...
	 * @param params
	 *            The parameter Map which contains key-value pairs.
	 * @throws PluginException
	 *             If something goes wrong
	 * @return True, if the condition holds.
	 */
	public static boolean ifInclude(WikiContext context, Map params)
			throws PluginException {
		...
		if (page != null) {
			String content = context.getEngine()
					.getPureText(page, WikiProvider.LATEST_VERSION).trim();
	...
	private static boolean checkGroup(WikiContext context, String group) {
		...
		for (int i = 0; i < groupList.length; i++) {
			...
			Principal g = mgr.resolvePrincipal(gname);

			include |= mgr.isUserInRole(context.getWikiSession(), g) ^ invert;
		}
		return include;
	}
	...
	private static boolean checkIP(WikiContext context, String ipaddr) {
		...
		for (int i = 0; i < list.length; i++) {
			...
			include |= ipaddr.equals(context.getHttpRequest().getRemoteAddr())
					^ invert;
		}
		return include;
	}
