 ...
 * <li><b>show</b> - value is either "pages" (default) or "count". When "count"
 * is specified, shows only the count of pages which match. (since 2.8)</li>
 ...
 * Options are 'human', 'java', 'locale' or a <code>RuleBasedCollator</code>
 * rule string. (since 2.8.3)</li>
 * </ul>
 * 
 */
public abstract class AbstractReferralPlugin implements WikiPlugin {
     ...
     *            the wiki context
     * @param params
     *            parameters for initializing the plugin
    ...
    public void initialize(WikiContext context, Map params)
        ...
        m_maxwidth = TextUtil.parseIntParameter(
                (String) params.get(PARAM_MAXWIDTH), Integer.MAX_VALUE);
        if (m_maxwidth < 0)
            m_maxwidth = 0;
        ...
        if (s != null) {
            try {
                ...
                throw new PluginException(
                        "Exclude-parameter has a malformed pattern: "
                                + e.getMessage());
            }
        }
        ...
        if (s != null) {
            try {
                ...
                throw new PluginException(
                        "Include-parameter has a malformed pattern: "
                                + e.getMessage());
            }
        }
     ...
     * 
     * @param links
     *            Collection to make into WikiText.
     * @param separator
    ...
    /**
     * A simple class that just cuts a String to a maximum length, adding three
     * dots after the cutpoint.
     */
    ...
    private void initSorter(WikiContext context, Map params) {
        ...
        if (order == null || order.length() == 0) {
            ...
            m_sorter = new PageSorter(
                    LocaleComparator.DEFAULT_LOCALE_COMPARATOR);
            ...
            try {
                ...
                log.info(
                        "Failed to parse requested collator - using default ordering",
                        pe);
                m_sorter = BeanHolder.getPageSorter();
            }
    }
