...
/**
 * Manages plugin classes. There exists a single instance of PluginManager per
 * each instance of WikiEngine, that is, each JSPWiki instance.
 * <P>
 ...
 * </pre>
 * 
 * The plugin class is "org.apache.wiki.plugin.FunnyPlugin", the parameters are
 * "foo" and "blob" (having values "bar" and "goo", respectively), and the
 * plugin body is then "abcdefghijklmnopqrstuvw\n01234567890". The plugin body
 * is accessible via a special parameter called "_body".
 * <p>
 * If the parameter "debug" is set to "true" for the plugin, JSPWiki will output
 * debugging information directly to the page if there is an exception.
 * <P>
 * The class name can be shortened, and marked without the package. For example,
 * "FunnyPlugin" would be expanded to "org.apache.wiki.plugin.FunnyPlugin"
 ...
 * </pre>
 * 
 * it is possible to shorten this quite a lot, by skipping the INSERT, and WHERE
 * words, and dropping the package name. For example:
 * 
 * <pre>
 ...
 * 
 * <p>
 * Plugin can implement multiple interfaces to let JSPWiki know at which stages
 * they should be invoked:
 * <ul>
 * <li>InitializablePlugin: If your plugin implements this interface, the
 * initialize()-method is called once for this class before any actual execute()
 * methods are called. You should use the initialize() for e.g. precalculating
 * things. But notice that this method is really called only once during the
 * entire WikiEngine lifetime. The InitializablePlugin is available from 2.5.30
 * onwards.</li>
 * <li>ParserStagePlugin: If you implement this interface, the executeParse()
 * method is called when JSPWiki is forming the DOM tree. You will receive an
 * incomplete DOM tree, as well as the regular parameters. However, since
 * JSPWiki caches the DOM tree to speed up later places, which means that
 * whatever this method returns would be irrelevant. You can do some DOM tree
 * manipulation, though. The ParserStagePlugin is available from 2.5.30 onwards.
 * </li>
 * <li>WikiPlugin: The regular kind of plugin which is executed at every
 * rendering stage. Each new page load is guaranteed to invoke the plugin,
 * unlike with the ParserStagePlugins.</li>
 * </ul>
 * 
 * @since 1.6.1
 */
public class PluginManager extends ModuleManager {
	 ...
	 * read stream of the plugin text (stored as a two-element int[], start and
	 * end resp.).
	 */
	...
	public void initializeProvider() {
		String packageNames = m_properties.getProperty(PROP_SEARCHPATH);
		...
		try {
			...
			log.fatal(
					"Internal error: someone messed with pluginmanager patterns.",
					e);
	 ...
	 * 
	 * @param enabled
	 *            True, if plugins should be globally enabled; false, if
	 *            disabled.
	 */
	 ...
	 * they are encountered on a WikiPage, and an empty string is returned in
	 * their place.
	 * 
	 ...
	 *            file (that is, "org.apache.wiki.plugin.Counter" or just plain
	 *            "Counter").
	 * 
	...
	public String execute(WikiContext context, String classname, Map params)
		...
		try {
			...
			if (pluginInfo == null) {
				pluginInfo = WikiPluginInfo
						.newInstance(findPluginClass(classname));
				registerPlugin(pluginInfo);
			}

			if (!checkCompatibility(pluginInfo)) {
				String msg = "Plugin '" + pluginInfo.getName()
						+ "' not compatible with this version of JSPWiki";
				log.info(msg);
				return msg;
			}
			...
			try {
				...
				throw new PluginException(rb.getString("plugin.error.failed"),
						t);
			}
	...
	/**
	 * Parses a plugin. Plugin commands are of the form: [{INSERT myplugin WHERE
	 * param1=value1, param2=value2}] myplugin may either be a class name or a
	 * plugin alias.
	 * <P>
	 ...
	 *            The current WikiContext.
	 * @param commandline
	 *            The full command line, including plugin name, parameters and
	 *            body.
	 * 
	...
	public String execute(WikiContext context, String commandline)
		...
		try {
			if (matcher.contains(commandline, m_pluginPattern)) {
				...
				String args = commandline
						.substring(
								res.endOffset(0),
								commandline.length()
										- (commandline.charAt(commandline
												.length() - 1) == '}' ? 1 : 0));
			...
			log.warn(msg, e);
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.missingparameter"), obArgs));
		} catch (IOException e) {
			String msg = "Zyrf.  Problems with parsing arguments: "
					+ commandline;
			log.warn(msg, e);
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.parsingarguments"), obArgs));
		}
	...
	public PluginContent parsePluginLine(WikiContext context,
			String commandline, int pos) throws PluginException {
		PatternMatcher matcher = new Perl5Matcher();
	...
	private void registerPlugins() {
		...
		try {
			//
			// Register all plugins which have created a resource containing its
			// properties.
			//
			...
			while (resources.hasMoreElements()) {
				...
				try {
					...
					for (Iterator i = plugins.iterator(); i.hasNext();) {
						...
						WikiPluginInfo pluginInfo = WikiPluginInfo.newInstance(
								className, pluginEl);
					...
					log.error("Couldn't load " + PLUGIN_RESOURCE_LOCATION
			...
			log.error("Couldn't load all " + PLUGIN_RESOURCE_LOCATION
					+ " resources", e);
		}
	}
	 ...
	 * @since
	 */
	// FIXME: This class needs a better interface to return all sorts of
	// possible
	...
	public static final class WikiPluginInfo extends WikiModuleInfo {
		...
		/**
		 * Creates a new plugin info object which can be used to access a
		 * plugin.
		 * 
		 * @param className
		 *            Either a fully qualified class name, or a "short" name
		 *            which is then checked against the internal list of plugin
		 *            packages.
		 * @param el
		 *            A JDOM Element containing the information about this
		 *            class.
		...
		protected void initializePlugin(WikiEngine engine) {
			if (!m_initialized) {
				// This makes sure we only try once per class, even if init
				// fails.
				m_initialized = true;
		 ...
		 */
		public WikiPlugin newPluginInstance() throws ClassNotFoundException,
	 ...
	 */
	// FIXME: This method needs to be reintegrated with execute() above, since
	// they
	...
	public void executeParse(PluginContent content, WikiContext context)
		...
		try {
			...
			WikiPluginInfo pluginInfo = m_pluginClassMap.get(content
					.getPluginName());

			if (pluginInfo == null) {
				pluginInfo = WikiPluginInfo.newInstance(findPluginClass(content
						.getPluginName()));
				registerPlugin(pluginInfo);
			}
			...
			if (plugin instanceof ParserStagePlugin) {
				((ParserStagePlugin) plugin).executeParser(content, context,
						params);
			}
		} catch (InstantiationException e) {
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.cannotinstantiate"), args), e);
		} catch (IllegalAccessException e) {
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.notallowed"), args), e);
		} catch (ClassNotFoundException e) {
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.couldnotfind"), args));
		} catch (ClassCastException e) {
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.notawikiplugin"), args), e);
		} catch (Exception e) {
			throw new PluginException(MessageFormat.format(
					rb.getString("plugin.error.instantationfailed"), args), e);
		}
	}
}
