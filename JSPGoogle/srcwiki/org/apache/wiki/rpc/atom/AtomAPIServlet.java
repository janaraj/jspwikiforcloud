...
public class AtomAPIServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (page == null) {
                throw new ServletException("Page " + blogid
                        + " does not exist, cannot add blog post.");
            }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            throw new ServletException(
                    "Internal problem - whack Janne on the head to get a better error report",
                    e);
        }
    ...
    private Entry getBlogEntry(String entryid) throws ProviderException {
        ...
        entry.setAuthor(SyndicationFactory.createPerson(page.getAuthor(), null,
                null));
    ...
    private Feed listBlogs() throws ProviderException, IOException {
        ...
        for (Iterator i = pages.iterator(); i.hasNext();) {
            ...
            log.debug(p.getName() + " = "
                    + p.getAttribute(WeblogPlugin.ATTR_ISWEBLOG));
            ...
            String title = TextUtil.replaceEntities(BlogUtil
                    .getSiteName(context));

            Link postlink = createLink("service.post", m_engine.getBaseURL()
                    + "atom/" + encodedName, title);

            Link editlink = createLink("service.edit", m_engine.getBaseURL()
                    + "atom/" + encodedName, title);

            Link feedlink = createLink("service.feed", m_engine.getBaseURL()
                    + "atom.jsp?page=" + encodedName, title);
