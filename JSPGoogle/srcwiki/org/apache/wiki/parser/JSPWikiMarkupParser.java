 ...
 * heart and soul of JSPWiki : make sure you test properly anything that is
 * added, or else it breaks down horribly.
 * 
 * @since 2.4
 */
public class JSPWikiMarkupParser extends MarkupParser {
    ...
    /**
     * The value for anchor element <tt>class</tt> attributes when used for wiki
     * page (normal) links. The value is "wikipage".
     */
    public static final String CLASS_WIKIPAGE = "wikipage";

    /**
     * The value for anchor element <tt>class</tt> attributes when used for edit
     * page links. The value is "createpage".
     */
    public static final String CLASS_EDITPAGE = "createpage";

    /**
     * The value for anchor element <tt>class</tt> attributes when used for
     * interwiki page links. The value is "interwiki".
     */
    ...
    private StringBuilder m_genlistBulletBuffer = new StringBuilder(10); // stores
                                                                         // the
                                                                         // #
                                                                         // and
                                                                         // *
    ...
    /**
     * If true, all outward attachment info links have a small link image
     * appended.
     */
    ...
    /**
     * If true, then generate special output for wysiwyg editing in certain
     * cases
     */
    ...
    /**
     * This list contains all IANA registered URI protocol types as of September
     * 2004 + a few well-known extra types.
     * 
     ...
     */
    static final String[] EXTERNAL_LINKS = { "http:", "ftp:", "https:",
            "mailto:", "news:", "file:", "rtsp:", "mms:", "ldap:", "gopher:",
            "nntp:", "telnet:", "wais:", "prospero:", "z39.50s", "z39.50r",
            "vemmi:", "imap:", "nfs:", "acap:", "tip:", "pop:", "dav:",
            "opaquelocktoken:", "sip:", "sips:", "tel:", "fax:", "modem:",
            "soap.beep:", "soap.beeps", "xmlrpc.beep", "xmlrpc.beeps", "urn:",
            "go:", "h323:", "ipp:", "tftp:", "mupdate:", "pres:", "im:",
            "mtqp", "smb:" };
    ...
    private static final String[] CLASS_TYPES = { CLASS_WIKIPAGE,
            CLASS_EDITPAGE, "", "footnote", "footnoteref", "", "external",
            CLASS_INTERWIKI, "external", CLASS_WIKIPAGE, "attachment" };
     ...
     * 
     * @param context
     *            The WikiContext which controls the parsing
     * @param in
    ...
    private void initialize() {
        ...
        // really expensive
        //
        compiledpatterns = (List<Pattern>) m_engine
                .getAttribute(INLINE_IMAGE_PATTERNS);
        ...
        if (m_camelCasePattern == null) {
            try {
                ...
                throw new InternalWikiException(
                        "Faulty camelcasepattern in TranslatorReader");
            }
        ...
        if (cclinks != null) {
            ...
            m_camelCaseLinks = TextUtil.getBooleanProperty(props,
                    PROP_CAMELCASELINKS, m_camelCaseLinks);
        }

        Boolean wysiwygVariable = (Boolean) m_context
                .getVariable(RenderingManager.WYSIWYG_EDITOR_MODE);
        if (wysiwygVariable != null) {
            m_wysiwygEditorMode = wysiwygVariable.booleanValue();
        }

        m_plainUris = getLocalBooleanProperty(m_context, props, PROP_PLAINURIS,
                m_plainUris);
        m_useOutlinkImage = getLocalBooleanProperty(m_context, props,
                PROP_USEOUTLINKIMAGE, m_useOutlinkImage);
        m_useAttachmentImage = getLocalBooleanProperty(m_context, props,
                PROP_USEATTACHMENTIMAGE, m_useAttachmentImage);
        m_allowHTML = getLocalBooleanProperty(m_context, props,
                MarkupParser.PROP_ALLOWHTML, m_allowHTML);

        m_useRelNofollow = getLocalBooleanProperty(m_context, props,
        ...
        if (userMgr.getUserDatabase() == null || mgr == null) {
            disableAccessRules();
        }
    ...
    /**
     * This is just a simple helper method which will first check the context if
     * there is already an override in place, and if there is not, it will then
     * check the given properties.
     * 
     * @param context
     *            WikiContext to check first
     * @param props
     *            Properties to check next
     * @param key
     *            What key are we searching for?
     * @param defValue
     *            Default value for the boolean
     * @return True or false
     */
    private static boolean getLocalBooleanProperty(WikiContext context,
            Properties props, String key, boolean defValue) {
     ...
     * 
     * @param engine
     *            The WikiEngine from which the patterns are read.
     */
     ...
     *            Chain to call
     * @param text
     *            Text that should be passed to the mutate() method of each of
     *            the mutators in the chain.
    ...
    /**
     * Creates a JDOM anchor element. Can be overridden to change the URL
     * creation, if you really know what you are doing.
     * 
     * @param type
     *            One of the types above
     * @param link
     *            URL to which to link to
     * @param text
     *            Link text
     * @param section
     *            If a particular section identifier is required.
     * @return An A element.
     * @since 2.4.78
     */
    ...
    private Element makeLink(int type, String link, String text,
            String section, Iterator attributes) {
        Element el = null;
        ...
        ResourceBundle rb = m_context
                .getBundle(InternationalizationManager.CORE_BUNDLE);
        Object[] args = { link };

        switch (type) {
        case READ:
            el = createAnchor(READ, m_context.getURL(WikiContext.VIEW, link),
                    text, section);
            break;

        case EDIT:
            el = createAnchor(EDIT, m_context.getURL(WikiContext.EDIT, link),
                    text, "");
            el.setAttribute(
                    "title",
                    MessageFormat.format(
                            rb.getString("markupparser.link.create"), args));
            break;
        ...
        //
        case LOCALREF:
            el = createAnchor(LOCALREF, "#ref-" + m_context.getName() + "-"
                    + link, "[" + text + "]", "");
            break;
        ...
        case LOCAL:
            ...
            el.setAttribute("name",
                    "ref-" + m_context.getName() + "-" + link.substring(1));
            el.addContent("[" + text + "]");
            break;
     ...
     */
    private static final String[] BLOCK_ELEMENTS = { "address", "blockquote",
            "div", "dl", "fieldset", "form", "h1", "h2", "h3", "h4", "h5",
            "h6", "hr", "noscript", "ol", "p", "pre", "table", "ul" };
    ...
    private String peekAheadLine() throws IOException {
        ...
        if (s.length() > PUSHBACK_BUFFER_SIZE) {
            log.warn("Line is longer than maximum allowed size ("
    ...
    /**
     * Writes HTML for error message. Does not add it to the document, you have
     * to do it yourself.
     * 
     * @param error
     *            The error string.
    ...
    public static Element makeError(String error) {
        return new Element("span").setAttribute("class", "error").addContent(
                error);
    }
    ...
    private int flushPlainText() {
        ...
        if (numChars > 0) {
            ...
            try {
                ...
                //
                // 1) Only when the length of the buffer is big enough, we try
                // to do the match
                //
                ...
                if (m_camelCaseLinks && !m_isEscaping && buf.length() > 3) {
                    ...
                    while (m_camelCaseMatcher.contains(buf, m_camelCasePattern)) {
                        ...
                        String firstPart = buf.substring(0,
                        ...
                        //
                        // Check if the user does not wish to do URL or WikiWord
                        // expansion
                        //
                ...
                //
                m_currentElement.addContent(makeError(cleanupSuspectData(e
                        .getMessage())));
            }
        }
     ...
     */
    // Keep sorted.
    private static final String[] EMPTY_ELEMENTS = { "area", "base", "br",
            "col", "hr", "img", "input", "link", "meta", "p", "param" };
     ...
     * 
     * @param s
     * @return The new current element, or null, if there was no such element in
     *         the entire stack.
     */
    private Element popElement(String s) {
        ...
        while (currEl.getParentElement() != null) {
            if (currEl.getName().equals(s) && !currEl.isRootElement()) {
                ...
                // trick the JDOM generator into not generating an empty
                // element,
                // by putting an empty string between the tags. Yes, it's a
                // kludge
                // but what'cha gonna do about it. :-)
                //

                if (flushedBytes == 0
                        && Arrays.binarySearch(EMPTY_ELEMENTS, s) < 0) {
                    currEl.addContent("");
                }
    ...
    /**
     * Reads the stream until it meets one of the specified ending characters,
     * or stream end. The ending character will be left in the stream.
     */
     ...
     * stream, returning then the result as a String.
     */
    ...
    /**
     * Does a lazy init. Otherwise, we would get into a situation where
     * HTMLRenderer would try and boot a TranslatorReader before the
     * TranslatorReader it is contained by is up.
     */
    private JSPWikiMarkupParser getCleanTranslator() {
        if (m_cleanTranslator == null) {
            WikiContext dummyContext = new WikiContext(m_engine,
                    m_context.getHttpRequest(), m_context.getPage());
    ...
    /**
     * Modifies the "hd" parameter to contain proper values. Because an "id" tag
     * may only contain [a-zA-Z0-9:_-], we'll replace the % after url encoding
     * with '_'.
     * <p>
    ...
    private String makeSectionTitle(String title) {
        ...
        try {
            ...
            throw new InternalWikiException(
                    "CleanTranslator not working as expected, when cleaning title"
                            + e.getMessage());
        }
     ...
     * 
     * @param level
     *            The level of the heading. @see Heading
     * @param title
     *            the title for the heading
     * @param hd
     *            a List to which heading should be added
    ...
    public Element makeHeading(int level, String title, Heading hd) {
        ...
        case Heading.HEADING_SMALL:
            el = new Element("h4").setAttribute("id",
                    makeHeadingAnchor(pageName, outTitle, hd));
            break;

        case Heading.HEADING_MEDIUM:
            el = new Element("h3").setAttribute("id",
                    makeHeadingAnchor(pageName, outTitle, hd));
            break;

        case Heading.HEADING_LARGE:
            el = new Element("h2").setAttribute("id",
                    makeHeadingAnchor(pageName, outTitle, hd));
            break;
    ...
    /**
     * Returns an element for the external link image (out.png). However, this
     * method caches the URL for the lifetime of this MarkupParser, because it's
     * commonly used, and we'll end up with possibly hundreds our thousands of
     ...
     * because of the way that flushPlainText() works, it already encodes all of
     * the XML entities. But so does WikiContext.getURL(), so we have to do a
     * reverse-replace here, so that it can again be replaced in makeLink.
     * <p>
    ...
    private Element handleImageLink(String reallink, String link,
            boolean hasLinkText) {
...
//            acl = m_engine.getAclManager().parseAcl(page, ruleLine);
            acl = aManager.parseAcl(page, ruleLine);
    ...
    private Element handleMetadata(String link) {
        ...
        try {
            ...
            if (name.length() > 0 && val.length() > 0) {
                val = BeanHolder.getVariableManager().expandVariables(
                        m_context, val);
            ...
            ResourceBundle rb = m_context
                    .getBundle(InternationalizationManager.CORE_BUNDLE);
            Object[] args = { link };
            return makeError(MessageFormat.format(
                    rb.getString("markupparser.error.invalidset"), args));
        }
    ...
    private Element handleHyperlinks(String linktext, int pos) {
        ...
        if (PluginManager.isPluginLink(linktext)) {
            try {
                PluginContent pluginContent = BeanHolder.getPluginManager()
                        .parsePluginLine(m_context, linktext, pos);
                //
                ...
                if (!m_wysiwygEditorMode) {
                    ResourceBundle rbPlugin = m_context
                            .getBundle(WikiPlugin.CORE_PLUGINS_RESOURCEBUNDLE);
        ...
        try {
            ...
            if (VariableManager.isVariableLink(linktext)) {
                ...
                // is using, so you'll have to write the entire name as it
                // appears
                // in the URL. Bugger.
                ...
                if (m_wysiwygEditorMode) {
                    ...
                    if (urlReference != null) {
                        ...
                        Object[] args = { extWiki };
                        addElement(makeError(MessageFormat
                                .format(rb
                                        .getString("markupparser.error.nointerwikiref"),
                                        args)));
                    }
                }
                ...
                if (attachment != null) {
                    ...
                    if ((matchedLink = linkExists(linkref)) != null) {
                        String sectref = "section-"
                                + m_engine.encodeName(matchedLink) + "-"
                                + wikifyLink(namedSection);
            ...
            addElement(makeError(MessageFormat.format(
                    rb.getString("markupparser.error.parserfailure"), args)));
        }
    ...
    /**
     * Pushes back any string that has been read. It will obviously be pushed
    ...
    private Element handleOpenbrace(boolean isBlock) throws IOException {
        ...
        if (ch == '{') {
            ...
            if (ch2 == '{') {
                ...
                return pushElement(new Element("span").setAttribute("style",
    ...
    /**
     * Reads the stream until the next EOL or EOF. Note that it will also read
     * the EOL from the stream.
     */
    ...
    /**
     * Starts a block level element, therefore closing a potential open
     * paragraph tag.
     */
    ...
    /**
     * Like original handleOrderedList() and handleUnorderedList() however
     * handles both ordered ('#') and unordered ('*') mixed together.
     */
    ...
    private Element handleGeneralList() throws IOException {
        ...
        // String strBulletsRaw = strBullets; // to know what was original
        ...
        if (m_allowPHPWikiStyleLists) {
            ...
            if (!(strBullets.substring(0, Math.min(numBullets, m_genlistlevel))
                    .equals(m_genlistBulletBuffer.substring(0,
                            Math.min(numBullets, m_genlistlevel))))) {
                if (numBullets <= m_genlistlevel) {
                    // Substitute all but the last character (keep the expressed
                    // bullet preference)
                    strBullets = (numBullets > 1 ? m_genlistBulletBuffer
                            .substring(0, numBullets - 1) : "")
        ...
        if (strBullets.substring(0, Math.min(numBullets, m_genlistlevel))
            ...
            if (numBullets > m_genlistlevel) {
                pushElement(new Element(
                        getListType(strBullets.charAt(m_genlistlevel++))));
                ...
                for (; m_genlistlevel > numBullets; m_genlistlevel--) {
                    ...
                    popElement(getListType(m_genlistBulletBuffer
                            .charAt(m_genlistlevel - 1)));
                    if (m_genlistlevel > 0) {
                        popElement("li");
                    }
            ...
            while (numEqualBullets < numCheckBullets) {
                ...
                if (strBullets.charAt(numEqualBullets) == m_genlistBulletBuffer
                        .charAt(numEqualBullets))
                    numEqualBullets++;
            ...
            pushElement(new Element(
                    getListType(strBullets.charAt(numEqualBullets++))));
    ...
    private Element handleOpenbracket() throws IOException {
        ...
        //
        // If the link is never finished, do some tricks to display the rest of
        // the line
        // unchanged.
        //
        if (ch == -1) {
    ...
    private Element handleDiv(boolean newLine) throws IOException {
        ...
        if (ch == '%') {
            ...
            if (ch == '(') {
                ...
                //
                // Pop out only spaces, so that the upcoming EOL check does not
                // check the
                // next line.
                //
                if (ch == '\n' || ch == '\r') {
                ...
                try {
                    ...
                    log.debug("Page '" + m_context.getName()
            ...
            try {
                ...
                if (style != null && style.indexOf("javascript:") != -1) {
                    log.debug("Attempt to output javascript within CSS:"
                            + style);
                    ResourceBundle rb = m_context
                            .getBundle(InternationalizationManager.CORE_BUNDLE);
                    return addElement(makeError(rb
                            .getString("markupparser.error.javascriptattempt")));
                }
    ...
    private Element handleBar(boolean newLine) throws IOException {
        ...
        if (newLine) {
            if (!m_istable) {
                startBlockLevel();
                el = pushElement(new Element("table").setAttribute("class",
                        "wikitable").setAttribute("border", "1"));
                m_istable = true;
                m_rowNum = 0;
            }

            m_rowNum++;
            Element tr = (m_rowNum % 2 != 0) ? new Element("tr").setAttribute(
                    "class", "odd") : new Element("tr");
            el = pushElement(tr);
        }
    ...
    private void fillBuffer(Element startElement) throws IOException {
        ...
        while (!quitReading) {
            ...
            try {
                ...
                log.info("Page " + m_context.getPage().getName()
                        + " contains data which cannot be added to DOM tree: "
                        + e.getMessage());
     ...
     * you think this was a wiki markup element, and IGNORE, if you think we
     * should ignore this altogether.
     * <p>
     * To add your own MarkupParser, you can override this method, but it is
     * recommended that you call super.parseToken() as well to gain advantage of
     * JSPWiki's own markup. You can call it at the start of your own
     * parseToken() or end - it does not matter.
     * 
     * @param ch
     *            The character under investigation
    ...
    private void paragraphify(Element rootElement) {
        ...
        if (rootElement.getChild("p") != null) {
            ...
            if (ls.size() > 0) {
                ...
                //
                if (newel.getTextTrim().length() > 0
                        || !newel.getChildren().isEmpty())
    ...
    /**
     * Compares two Strings, and if one starts with the other, then returns
     * null. Otherwise just like the normal Comparator for strings.
     * 
     * @since
     */
    private static class StartingComparator implements Comparator<String> {
        public int compare(String s1, String s2) {
            if (s1.length() > s2.length()) {
                ...
                if (s2.startsWith(s1) && s1.length() > 1)
                    return 0;
            }
