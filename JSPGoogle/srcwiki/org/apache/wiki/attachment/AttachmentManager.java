...
/**
 * Provides facilities for handling attachments. All attachment handling goes
 * through this class.
 * <p>
 * The AttachmentManager provides a facade towards the current
 * WikiAttachmentProvider that is in use. It is created by the WikiEngine as a
 * singleton object, and can be requested through the WikiEngine.
 * 
 * @since 1.9.28
 */
...
public class AttachmentManager extends AbstractWikiProvider {
     ...
     * 
     * @return A boolean value indicating whether attachment functionality is
     *         enabled.
     */
    ...
    /**
     * Figures out the full attachment name from the context and attachment
     * name.
     * 
     * @param context
    ...
    public Attachment getAttachmentInfo(WikiContext context,
            String attachmentname) throws ProviderException {
        return getAttachmentInfo(context, attachmentname,
    ...
    public Attachment getAttachmentInfo(WikiContext context,
            String attachmentname, int version) throws ProviderException {
        if (m_provider == null) {
            return null;
        }
        ...
        if (att == null) {
            att = m_provider.getAttachmentInfo(currentPage, attachmentname,
                    version);
        }
    ...
    /**
     * Returns the list of attachments associated with a given wiki page. If
     * there are no attachments, returns an empty Collection.
     * 
     * @param wikipage
    ...
    public Collection listAttachments(WikiPage wikipage)
        ...
        //
        // This is just a sanity check; all of our providers return a
        // Collection.
        //
    ...
    /**
     * Returns true, if the page has any attachments at all. This is a
     * convinience method.
     * 
     * 
     * @param wikipage
     ...
     * @param att
     *            Attachment
     * @return An InputStream to read from. May return null, if attachments are
     *         disabled.
     * @throws IOException
     ...
     */
    public InputStream getAttachmentStream(Attachment att) throws IOException,
            ProviderException {
        return getAttachmentStream(null, att);
    }

    /**
     * Returns an attachment stream using the particular WikiContext. This
     * method should be used instead of getAttachmentStream(Attachment), since
     * it also allows the DynamicAttachments to function.
     * 
     * @param ctx
     *            The Wiki Context
     * @param att
     *            The Attachment to find
     * @return An InputStream. May return null, if attachments are disabled. You
     *         must take care of closing it.
     * @throws ProviderException
     *             If the backend fails due to some reason
     * @throws IOException
     *             If the stream cannot be opened
     */
    public InputStream getAttachmentStream(WikiContext ctx, Attachment att)
        ...
        if (att instanceof DynamicAttachment) {
            return ((DynamicAttachment) att).getProvider().getAttachmentData(
                    ctx, att);
        }
    ...
    /**
     * Finds a DynamicAttachment. Normally, you should just use
     * getAttachmentInfo(), since that will find also DynamicAttachments.
     * 
     * @param name
     *            The name of the attachment to look for
    ...
    /**
     * Stores an attachment that lives in the given file. If the attachment did
     * not exist previously, this method will create it. If it did exist, it
     * stores a new version.
     * 
     * @param att
     *            Attachment to store this under.
     * @param source
     *            A file to read from.
     * 
     * @throws IOException
     *             If writing the attachment failed.
     * @throws ProviderException
    ...
    /**
     * Stores an attachment directly from a stream. If the attachment did not
     * exist previously, this method will create it. If it did exist, it stores
     * a new version.
     * 
     * @param att
     *            Attachment to store this under.
     * @param in
     *            InputStream from which the attachment contents will be read.
     * 
     * @throws IOException
     *             If writing the attachment failed.
     * @throws ProviderException
    ...
    public void storeAttachment(Attachment att, InputStream in)
        ...
        BeanHolder.getReferenceManager().updateReferences(att.getName(),
     ...
     * 
     * @param attachmentName
     *            A fully qualified name of the attachment.
     * 
    ...
    public void deleteAttachment(Attachment att) throws ProviderException {
        ...
        BeanHolder.getSearchManager().pageRemoved(att);

        BeanHolder.getReferenceManager().clearPageEntries(att.getName());
    ...
    static String validateFileName(String filename) throws WikiException {
        ...
        // to remove it here by simply splitting along slashes and then taking
        // the path.
        //
