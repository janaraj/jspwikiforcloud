...
/**
 * The master class for generating different kinds of Feeds (including RSS1.0,
 * 2.0 and Atom).
 * <p>
 * This class can produce quite a few different styles of feeds. The following
 * modes are available:
 * 
 * <ul>
 ...
 * RecentChanges-style feed, where each page is only listed once, even if it has
 * changed multiple times.</li>
 * <li><b>blog</b> - Each page change is assumed to be a blog entry, so no diffs
 * are produced, but the page content is always completely in the entry in
 * rendered HTML.</li>
 * 
 * @since 1.7.5.
 */
// FIXME: Limit diff and page content size.
// FIXME3.0: This class would need a bit of refactoring. Method names, e.g. are
// confusing.
@SuppressWarnings("serial")
public class RSSGenerator implements Serializable {
	static Log log = LogFactory.getLog(RSSGenerator.class);
	...
	/**
	 * Defines the property name for the RSS channel description. Default value
	 * for the channel description is an empty string.
	 * 
	 * @since 1.7.6.
	 */
	...
	/**
	 * Defines the property name for the RSS channel language. Default value for
	 ...
	 * <tt>{@value org.apache.wiki.WikiEngine#PROP_BASEURL}</tt>.
	 * 
	 * @param engine
	 *            The WikiEngine.
	 * @param properties
	 ...
	 *             If something is missing from the given property set.
	 */
	public RSSGenerator(WikiEngine engine, Properties properties)
		...
		if (engine.getBaseURL() == null || engine.getBaseURL().length() == 0) {
			throw new NoRequiredPropertyException(
					"RSS requires jspwiki.baseURL to be set!",
	...
	private String getAttachmentDescription(Attachment att) {
		...
		if (att.getVersion() != 1) {
			sb.append(author + " uploaded a new version of this attachment on "
					+ att.getLastModified());
		} else {
			sb.append(author + " created this attachment on "
					+ att.getLastModified());
		}

		sb.append("<br /><hr /><br />");
		sb.append("Parent page: <a href=\""
				+ m_engine.getURL(WikiContext.VIEW, att.getParentName(), null,
						true) + "\">" + att.getParentName() + "</a><br />");
	...
	private String getPageDescription(WikiPage page) {
		...
		if (page.getVersion() > 1) {
			String diff = m_engine.getDiff(ctx, page.getVersion() - 1, // FIXME:
																		// Will
																		// fail
																		// when
																		// non-contiguous
			...
			buf.append(author + " changed this page on "
					+ page.getLastModified() + ":<br /><hr /><br />");
			buf.append(diff);
		} else {
			buf.append(author + " created this page on "
					+ page.getLastModified() + ":<br /><hr /><br />");
	...
	/**
	 * Generates the RSS resource. You probably want to output this result into
	 * a file or something, or serve as output from a servlet.
	 * 
	 ...
	 * @since 2.3.15
	 * @param mode
	 *            the RSS mode: {@link #RSS10}, {@link #RSS20} or {@link #ATOM}.
	 ...
	 */
	public String generateFeed(WikiContext wikiContext, List changed,
			String mode, String type) throws ProviderException,
	...
	/**
	 * Turns RSS generation on or off. This setting is used to set the "enabled"
	 * flag only for use by callers, and does not actually affect whether the
	 * {@link #generate()} or
	 * {@link #generateFeed(WikiContext, List, String, String)} methods output
	 * anything.
	 * 
	 * @param enabled
	...
	protected String generateFullWikiRSS(WikiContext wikiContext, Feed feed) {
		...
		for (Iterator i = changed.iterator(); i.hasNext() && items < 15; items++) {
			...
			if (!mgr.checkPermission(session, new PagePermission(page,
	...
	protected String generateWikiPageRSS(WikiContext wikiContext, List changed,
		...
		String channelDescription = m_engine.getVariable(wikiContext,
	 ...
	 * 
	 * @param wikiContext
	 *            The WikiContext, as usual.
	 * @param changed
	 *            A list of the changed pages.
	 * @param feed
	 *            A valid Feed object. The feed will be used to create the
	 *            RSS/Atom, depending on which kind of an object you want to put
	 *            in it.
	...
	protected String generateBlogRSS(WikiContext wikiContext, List changed,
			Feed feed) throws ProviderException {
		...
		for (Iterator i = changed.iterator(); i.hasNext() && items < 15; items++) {
			...
			String pageText = m_engine.getPureText(page.getName(),
