 ...
 * </UL>
 * 
 * This is a quick'n'dirty approach without any finesse in storage and searching
 * algorithms; we trust java.util.*.
 * <P>
 * This class contains two HashMaps, m_refersTo and m_referredBy. The first is
 * indexed by WikiPage names and contains a Collection of all WikiPages the page
 * refers to. (Multiple references are not counted, naturally.) The second is
 * indexed by WikiPage names and contains a Set of all pages that refer to the
 * indexing page. (Notice - the keys of both Maps should be kept in sync.)
 * <P>
 * When a page is added or edited, its references are parsed, a Collection is
 * received, and we crudely replace anything previous with this new Collection.
 * We then check each referenced page name and make sure they know they are
 * referred to by the new page.
 * <P>
 * Based on this information, we can perform non-optimal searches for e.g.
 * unreferenced pages, top ten lists, etc.
 * <P>
...
public class ReferenceManager extends BasicPageFilter implements WikiProvider,
	...
	/**
	 * Maps page wikiname to a Set of referring pages. The Set must contain
	 * Strings. Non-existing pages (a reference exists, but not a file for the
	 * page contents) may have an empty Set in m_referredBy.
	 */
	 ...
	 * 
	 * @param pages
	 *            A collection of all pages you want to be included in the
	 *            reference count.
	 * @since 2.2
	...
	public void initializePages(Collection pages) throws ProviderException {
		...
		try {
			...
			log.info("Unable to unserialize old refmgr information, rebuilding database: "
					+ e.getMessage());
			buildKeyLists(pages);

			// Scan the existing pages from disk and update references in the
			// manager.
			Iterator it = pages.iterator();
			while (it.hasNext()) {
				WikiPage page = (WikiPage) it.next();
	...
	/**
	 * Reads the serialized data from the disk back to memory. Returns the date
	 * when the data was last written on disk
	 */
	@SuppressWarnings("unchecked")
	private synchronized long unserializeFromDisk() throws IOException,
		...
		try {
			...
			if (ver != serialVersionUID) {
				throw new IOException(
						"File format has changed; I need to recalculate references.");
			}
	...
	private String getHashFileName(String pageName)
		...
		try {
			...
			throw new InternalWikiException(
					"AAAAGH!  UTF-8 is gone!  My eyes!  It burns...!");
		}
...
//			log.fatal("No MD5!?!");
	...
	private synchronized void serializeAttrsToDisk(WikiPage p) {
		...
		try {
			...
			out = oP.constructOutput(SERIALIZATION_DIR, getHashFileName(p.getName()), entries.size() == 0);
			if (out == null) { return; }
//			File f = new File(m_engine.getWorkDir(), SERIALIZATION_DIR);
...
//					new xjava.io.FileOutputStream(f)));
	 ...
	 * 
	 * @param page
	 *            Name of the page to remove from the maps.
	 */
	...
	private void pageRemoved(String pageName) {
		...
		if (refTo != null) {
			...
			while (itRefTo.hasNext()) {
				...
				if (refBy == null)
					throw new InternalWikiException("Refmgr out of sync: page "
							+ pageName + " refers to " + referredPageName
			...
			log.debug("Removing from m_refersTo HashMap key:value " + pageName
					+ ":" + m_refersTo.get(pageName));
		...
		try {
			IObjectPersist oP = m_engine.getObjectPersist();
			oP.constructOutput(SERIALIZATION_DIR, getHashFileName(pageName), true);
//			File f = new File(m_engine.getWorkDir(), SERIALIZATION_DIR);
...
//				f.delete();
	 ...
	 * <P>
	 * This is the method to call when a new page has been created and we want
	 * to a) set up its references and b) notify the referred pages of the
	 * references. Use this method during run-time.
	 * 
	 * @param page
	 *            Name of the page to update.
	 * @param references
	 *            A Collection of Strings, each one pointing to a page this page
	 *            references.
	 */
	...
	/**
	 * Cleans the 'referred by' list, removing references by 'referrer' to any
	 * other page. Called after 'referrer' is removed.
	 */
	 ...
	 * BEFORE calling updateReferences() with a full list of existing page
	 * names. It builds the refersTo and referredBy key lists, thus enabling
	 * updateReferences() to function correctly.
	 * <P>
	 * This method should NEVER be called after initialization. It clears all
	 * mappings from the reference tables.
	 * 
	 * @param pages
	 *            a Collection containing WikiPage objects.
	 */
	private synchronized void buildKeyLists(Collection pages) {
		...
		try {
			while (it.hasNext()) {
				WikiPage page = (WikiPage) it.next();
				// We add a non-null entry to referredBy to indicate the
				// referred page exists
				m_referredBy.put(page.getName(), new TreeSet<String>());
				// Just add a key to refersTo; the keys need to be in sync with
				// referredBy.
				m_refersTo.put(page.getName(), null);
			}
		} catch (ClassCastException e) {
			log.fatal(
					"Invalid collection entry in ReferenceManager.buildKeyLists().",
					e);
		}
	}

	/**
	 * Marks the page as referred to by the referrer. If the page does not exist
	 * previously, nothing is done. (This means that some page, somewhere, has a
	...
	private void updateReferredBy(String page, String referrer) {
		...
		/*
		 * if( page.equals( referrer ) ) { return; }
		 */
		// Neither are we interested if plural forms refer to each other.
		if (m_matchEnglishPlurals) {
			String p2 = page.endsWith("s") ? page.substring(0,
					page.length() - 1) : page + "s";
	 ...
	 * 
	 * @param pagename
	 *            Name of the page to clear references for.
	 */
	...
	/**
	 * Finds all references to non-existant pages. This requires a linear scan
	 * through m_refersTo values; each value must have a corresponding key entry
	 * in the reference Maps, otherwise such a page has never been created.
	 * <P>
	 * Returns a Collection containing Strings of unreferenced page names. Each
	 * non-existant page name is shown only once - we don't return information
	...
	/**
	 * Searches for the given page in the given Map, and returns the set of
	 * references. This method also takes care of English plural matching.
	 * 
	 * @param coll
	...
	/**
	 * Find all pages that refer to this page. Returns null if the page does not
	 * exist or is not referenced at all, otherwise returns a collection
	 * containing page names (String) that refer to this one.
	 * <p>
	 * 
	 * @param pagename
	 *            The page to find referrers for.
	...
	/**
	 * Returns all pages that refer to this page. Note that this method returns
	 * an unmodifiable Map, which may be abruptly changed. So any access to any
	 * iterator may result in a ConcurrentModificationException.
	 * <p>
	 * The advantages of using this method over findReferrers() is that it is
	 * very fast, as it does not create a new object. The disadvantages are that
	 * it does not do any mapping between plural names, and you may end up
	 * getting a ConcurrentModificationException.
	 * 
	 * @param pageName
	 *            Page name to query.
	 * @return A Set of Strings containing the names of all the pages that refer
	 *         to this page. May return null, if the page does not exist or has
	 ...
	 * <p>
	 * The Collection returned is unmutable, so you cannot change it. It does
	 ...
	 * ReferenceManager. The maps of the ReferenceManager are not synchronized,
	 * so someone could add/remove entries in them while the hashCode is being
	 * computed.
	 * 
	...
	// modifcaition occurs.
	// TODO: It is unnecessary to calculate the hashcode; it should be
	// calculated only
