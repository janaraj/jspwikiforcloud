...
public class RCSFileProvider extends AbstractFileProvider {
	...
	public void initialize(WikiEngine engine, Properties props)
		...
		m_checkoutVersionCommand = props.getProperty(PROP_CHECKOUTVERSION,
	...
	public WikiPage getPageInfo(String page, int version)
		...
		try {
			...
			cmd = TextUtil
					.replaceString(cmd, "%s", mangleName(page) + FILE_EXT);

			Process process = Runtime.getRuntime().exec(cmd, null,
					new File(getPageDirectory()));
			...
			while ((line = stdout.readLine()) != null) {
				if (matcher.contains(line, headpattern)) {
					...
					try {
						...
						if (vernum == version
								|| version == WikiPageProvider.LATEST_VERSION) {
							info.setVersion(vernum);
							found = true;
						}
					} catch (NumberFormatException e) {
						log.info(
								"Failed to parse version number from RCS log: ",
								e);
				...
				if (found && matcher.contains(line, notepattern)) {
					...
					info.setAttribute(WikiPage.CHANGENOTE,
							TextUtil.urlDecodeUTF8(result.group(1)));
				}
			}
	...
	public String getPageText(String page, int version)
		...
		try {
			...
			if (exitVal != 0 || checkedOutVersion == -1) {
				if (version == 1) {
					result = super.getPageText(page,
							WikiProvider.LATEST_VERSION);
				} else {
					throw new NoSuchVersionException("Page: " + page
							+ ", version=" + version);
				}
			} else {
				//
	...
	public void putPageText(WikiPage page, String text)
		...
		try {
			...
			cmd = TextUtil.replaceString(cmd, "%s", mangleName(pagename)
					+ FILE_EXT);
			cmd = TextUtil.replaceString(cmd, "%u",
					TextUtil.urlEncodeUTF8(author));
			cmd = TextUtil.replaceString(cmd, "%c",
					TextUtil.urlEncodeUTF8(changenote));
			log.debug("Command = '" + cmd + "'");
	...
	public List getVersionHistory(String page) {
		...
		try {
			...
			String cmd = TextUtil.replaceString(m_fullLogCommand, "%s",
		...
		if (rcsdir.exists() && rcsdir.isDirectory()) {
			...
			if (rcsfile.exists()) {
				if (rcsfile.delete() == false) {
					log.warn("Deletion of RCS file "
							+ rcsfile.getAbsolutePath() + " failed!");
				}
			} else {
