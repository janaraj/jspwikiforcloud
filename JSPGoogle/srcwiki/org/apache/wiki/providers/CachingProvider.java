...
public class CachingProvider extends AbstractWikiProvider implements WikiPageProvider, VersioningProvider {
	...
	public void initialize(WikiEngine engine, Properties properties)
		...
		m_expiryPeriod = TextUtil.getIntegerProperty(properties,
				PROP_CACHECHECKINTERVAL, m_expiryPeriod);
		...
		int capacity = TextUtil.getIntegerProperty(properties,
				PROP_CACHECAPACITY, DEFAULT_CACHECAPACITY);
		...
		// of the interface.
		//
		m_cache.addCacheEventListener(m_allCollector,
		...
		m_textCache = new Cache(true, false, false, false, OSCACHE_ALGORITHM,
				capacity);

		m_historyCache = new Cache(true, false, false, false,
				OSCACHE_ALGORITHM, capacity);
	...
	private WikiPage getPageInfoFromCache(String name)
		...
		try {
			WikiPage item = (WikiPage) m_cache.getFromCache(name,
					m_expiryPeriod);
			...
			refreshed = m_provider.getPageInfo(name,
					WikiPageProvider.LATEST_VERSION);
			...
			if (refreshed == null && cached != null) {
				...
				if (refreshed != null) {
					...
					throw new RepositoryModifiedException("Added: " + name,
							name);
				}
				...
				m_negCache.putInCache(name, name);
					...
					&& cached.getVersion() != refreshed.getVersion()) {
				// The newest version has been deleted, but older versions still
				// remain
				log.debug("Page " + cached.getName()
						+ " newest version deleted, reloading...");
				...
				return refreshed;
					...
					&& Math.abs(refreshed.getLastModified().getTime()
	...
	public boolean pageExists(String pageName, int version) {
		...
		//
		try {
			String isNonExistant = (String) m_negCache.getFromCache(pageName,
					m_expiryPeriod);
		...
		try {
			...
			// removed or
			// added.
			// TODO: This information would be available in the exception, but
			// we would
			// need to subclass.
			...
			log.info("Provider failed while trying to check if page exists: "
					+ pageName);
			return false;
		}
	...
	private String getTextFromCache(String pageName) throws ProviderException,
		...
		try {
			...
			if (text == null) {
				if (page != null) {
					text = m_provider.getPageText(pageName,
							WikiPageProvider.LATEST_VERSION);
	...
	public synchronized String getProviderInfo() {
		return "Real provider: " + m_provider.getClass().getName()
				+ ". Cache misses: " + m_cacheMisses + ". Cache hits: "
				+ m_cacheHits + ". History cache hits: " + m_historyCacheHits
