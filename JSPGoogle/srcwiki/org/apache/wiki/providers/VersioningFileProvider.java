...
/**
 * Provides a simple directory based repository for Wiki pages. Pages are held
 ...
 * 
 */
public class VersioningFileProvider extends AbstractFileProvider implements
        VersioningProvider {
    private static final Log log = LogFactory
            .getLog(VersioningFileProvider.class);
        ...
        if (!oldpages.exists()) {
            if (!oldpages.mkdirs()) {
                throw new WikiException(
                        "Failed to create page version directory "
                                + oldpages.getAbsolutePath());
            }
        } else {
            if (!oldpages.isDirectory()) {
                throw new WikiException(
                        "Page version directory is not a directory: "
                                + oldpages.getAbsolutePath());
            }
            if (!oldpages.canWrite()) {
                throw new WikiException(
                        "Page version directory is not writable: "
                                + oldpages.getAbsolutePath());
            }
        }
        log.info("Using directory " + oldpages.getAbsolutePath()
                + " for storing old versions of pages");
    }
    ...
    private File findOldPageDir(String page) {
        if (page == null) {
            throw new InternalWikiException(
                    "Page may NOT be null in the provider!");
        }
     ...
     * in that directory.
     * 
     * @return Latest version number in the repository, or -1, if there is no
     *         page in the repository.
     */

    // FIXME: This is relatively slow.
    /*
     * private int findLatestVersion( String page ) { File pageDir =
     * findOldPageDir( page );
     * 
     * String[] pages = pageDir.list( new WikiFileFilter() );
     * 
     * if( pages == null ) { return -1; // No such thing found. }
     * 
     * int version = -1;
     * 
     * for( int i = 0; i < pages.length; i++ ) { int cutpoint =
     * pages[i].indexOf( '.' ); if( cutpoint > 0 ) { String pageNum =
     * pages[i].substring( 0, cutpoint );
     * 
     * try { int res = Integer.parseInt( pageNum );
     * 
     * if( res > version ) { version = res; } } catch( NumberFormatException e )
    ...
    private Properties getPageProperties(String page) throws IOException {
        ...
        if (propertyFile.exists()) {
            ...
            // The loading
            // of a propertyfile is a cpu-intensive job. So now hold on to the
            // last propertyfile
    ...
    private void putPageProperties(String page, Properties properties)
        ...
        try {
            out = new xjava.io.FileOutputStream(propertyFile);
    ...
    private int realVersion(String page, int requestedVersion)
        ...
        if (requestedVersion == latest
            ...
            throw new NoSuchVersionException("Requested version "
                    + requestedVersion + ", but latest is " + latest);
        }
    ...
    public synchronized String getPageText(String page, int version)
        ...
        if (!pageFile.exists())
            throw new NoSuchVersionException("Version " + version
                    + "does not exist.");
    ...
    private String readFile(File pagedata) throws ProviderException {
        ...
        if (pagedata.exists()) {
            if (pagedata.canRead()) {
                ...
                log.warn("Failed to read page from '"
     ...
     * 
     * version pagedir olddir none empty empty 1 Main.txt (1) empty 2 Main.txt
     * (2) 1.txt 3 Main.txt (3) 1.txt, 2.txt
     */
    /**
     * {@inheritDoc}
     */
        ...
        try {
            ...
            if (oldFile != null && oldFile.exists()) {
                ...
                try {
                    ...
                    File pageFile = new File(pageDir,
                            Integer.toString(versionNumber) + FILE_EXT);
                    out = new BufferedOutputStream(
                            new xjava.io.FileOutputStream(pageFile));
            ...
            throw new ProviderException("Could not save page text: "
                    + e.getMessage());
        }
    }
     ...
     * does not remove any extra subdirectories from it. It will only touch
     * those files that it thinks to be WikiPages.
     * 
     * @param page
     *            {@inheritDoc}
     * @throws {@inheritDoc}
     ...
     * {@inheritDoc}
     * 
     * Deleting versions has never really worked, JSPWiki assumes that version
     * histories are "not gappy". Using deleteVersion() is definitely not
     * recommended.
     * 
     */
    public void deleteVersion(String page, int version)
        ...
        if (version == WikiPageProvider.LATEST_VERSION || version == latest
            ...
            File previousFile = new File(pageDir, Integer.toString(latest)
                    + FILE_EXT);
        ...
        if (pageFile.exists()) {
            ...
            throw new NoSuchVersionException("Page " + page + ", version="
                    + version);
        }
    }
    ...
    public Collection getAllPages() throws ProviderException {
        ...
        for (Iterator i = pages.iterator(); i.hasNext();) {
            ...
            WikiPage info = getPageInfo(page.getName(),
