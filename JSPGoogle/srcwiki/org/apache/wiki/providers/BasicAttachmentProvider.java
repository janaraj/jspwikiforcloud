 ...
 * </UL>
 */
public class BasicAttachmentProvider extends AbstractWikiProvider implements WikiAttachmentProvider {
	private String m_storageDir;

	/**
	 * The property name for where the attachments should be stored. Value is
	 * <tt>{@value}</tt>.
	 */
	...
	/**
	 * The property name for specifying which attachments are not cached. Value
	 * is <tt>{@value}</tt>.
	 */
	...
	public void initialize(WikiEngine engine, Properties properties)
		...
		if (!f.canWrite())
			throw new WikiException(
					"Cannot write to the attachment storage directory '"
							+ m_storageDir + "'");
	...
	private void putPageProperties(Attachment att, Properties properties)
		...
		OutputStream out = new xjava.io.FileOutputStream(propertyFile);
	...
	public void putAttachmentData(Attachment att, InputStream data)
		...
		try {
			...
			out = new xjava.io.FileOutputStream(newfile);
			...
			if (author == null) {
				author = "unknown"; // FIXME: Should be localized, but cannot
									// due to missing WikiContext
			}
				...
				for (int i = 0; i < attachments.length; i++) {
					...
					if (f.isDirectory()) {
						...
						if (attachmentName.endsWith(ATTDIR_EXTENSION)) {
							attachmentName = attachmentName.substring(
									0,
						...
						Attachment att = getAttachmentInfo(page,
								attachmentName, WikiProvider.LATEST_VERSION);
	...
	public List listAllChanged(Date timestamp) throws ProviderException {
		...
		for (int i = 0; i < pagesWithAttachments.length; i++) {
			...
			pageId = pageId.substring(0,
					pageId.length() - DIR_EXTENSION.length());
	...
	public List getVersionHistory(Attachment att) {
		...
		try {
			...
			for (int i = latest; i >= 1; i--) {
				Attachment a = getAttachmentInfo(
						new WikiPage(m_engine, att.getParentName()),
						att.getFileName(), i);
	...
	public void moveAttachmentsForPage(String oldParent, String newParent)
		...
		log.debug("Trying to move all attachments from " + srcDir + " to "
				+ destDir);
		...
		if (destDir.exists()) {
			log.error("Page rename failed because target dirctory " + destDir
					+ " exists");
		} else {
