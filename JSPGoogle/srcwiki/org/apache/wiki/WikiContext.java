...
/**
 * <p>
 * Provides state information throughout the processing of a page. A WikiContext
 * is born when the JSP pages that are the main entry points, are invoked. The
 * JSPWiki engine creates the new WikiContext, which basically holds information
 * about the page, the handling engine, and in which context (view, edit, etc)
 * the call was done.
 * </p>
 * <p>
 * A WikiContext also provides request-specific variables, which can be used to
 * communicate between plugins on the same page, or between different instances
 * of the same plugin. A WikiContext variable is valid until the processing of
 * the page has ended. For an example, please see the Counter plugin.
 * </p>
 * <p>
 * When a WikiContext is created, it automatically associates a
 * {@link WikiSession} object with the user's HttpSession. The WikiSession
 * contains information about the user's authentication status, and is consulted
 * by {@link #getCurrentUser()}. object
 * </p>
 * <p>
 * Do not cache the page object that you get from the WikiContext; always use
 * getPage()!
 * </p>
 * 
...
public class WikiContext implements Cloneable, Command {
	...
	public static final String INSTALL = WikiCommand.INSTALL
			.getRequestContext();
	...
	public static final String WORKFLOW = WikiCommand.WORKFLOW
			.getRequestContext();
	...
	public static final String MESSAGE = WikiCommand.MESSAGE
			.getRequestContext();
	...
	public static final String PREVIEW = PageCommand.PREVIEW
			.getRequestContext();

	/**
	 * User has an internal conflict, and does quite not know what to do. Please
	 * provide some counseling.
	 */
	public static final String CONFLICT = PageCommand.CONFLICT
			.getRequestContext();
	...
	public static final String COMMENT = PageCommand.COMMENT
			.getRequestContext();
	...
	public static final String CREATE_GROUP = WikiCommand.CREATE_GROUP
			.getRequestContext();

	/** User is deleting an existing group. */
	public static final String DELETE_GROUP = GroupCommand.DELETE_GROUP
			.getRequestContext();

	/** User is editing an existing group. */
	public static final String EDIT_GROUP = GroupCommand.EDIT_GROUP
			.getRequestContext();

	/** User is viewing an existing group */
	public static final String VIEW_GROUP = GroupCommand.VIEW_GROUP
			.getRequestContext();
	 ...
	 *             if <code>engine</code> or <code>command</code> are
	 *             <code>null</code>
	 */
	public WikiContext(WikiEngine engine, HttpServletRequest request,
			Command command) throws IllegalArgumentException {
		super();
		if (engine == null || command == null) {
			throw new IllegalArgumentException(
					"Parameter engine and command must not be null.");
		}
		...
		// Special case: retarget any empty 'view' PageCommands to the front
		// page
		...
		if (log.isDebugEnabled()) {
			HttpSession session = (request == null) ? null : request
					.getSession(false);
	 ...
	 * {@link org.apache.wiki.ui.Command#getName()} method. Calling classes can
	 * rely on the results of this method for looking up canonically-correct
	 ...
	 * 
	 * @param context
	 *            The request context (e.g. WikiContext.UPLOAD)
	 * @param page
	 *            The page to which to link
	 * @param params
	...
	public String getURL(String context, String page, String params) {
		boolean absolute = "absolute".equals(m_engine.getVariable(this,
	...
	public WikiContext deepClone() {
		try {
			...
			copy.m_template = m_template;
			copy.m_variableMap = (HashMap<String, Object>) m_variableMap
					.clone();
			copy.m_request = m_request;
			copy.m_session = m_session;
	...
	public static WikiContext findContext(PageContext pageContext) {
		HttpServletRequest request = (HttpServletRequest) pageContext
				.getRequest();
		WikiContext context = (WikiContext) request
				.getAttribute(WikiTagBase.ATTR_CONTEXT);
		return context;
	}

	/**
	 * Returns the permission required to successfully execute this context. For
	 * example, the a wiki context of VIEW for a certain page means that the
	 * PagePermission "view" is required for the page. In some cases, no
	...
	/**
	 * Associates a target with the current Command and returns the new targeted
	 * Command. If the Command associated with this WikiContext is already
	 * "targeted", it is returned instead.
	 * 
	...
	/**
	 * Checks whether the current user has access to this wiki context, by
	 * obtaining the required Permission ({@link #requiredPermission()}) and
	 * delegating the access check to
	 ...
	 * <code>false</code> otherwise. If access is allowed, the wiki context will
	 * be added to the request as an attribute with the key name
	 * {@link org.apache.wiki.tags.WikiTagBase#ATTR_CONTEXT}. Note that this
	 * method will automatically redirect the user to a login or error page, as
	 * appropriate, if access fails. This is NOT guaranteed to be default
	 ...
	 * {@link #requiredPermission()}) and delegating the access check to
	 * {@link org.apache.wiki.auth.AuthorizationManager#checkPermission(WikiSession, Permission)}
	 * . If the user is allowed, this method returns <code>true</code>;
	 * <code>false</code> otherwise. If access is allowed, the wiki context will
	 * be added to the request as attribute with the key name
	 * {@link org.apache.wiki.tags.WikiTagBase#ATTR_CONTEXT}.
	 * 
	 * @return the result of the access check
	 * @param response
	 ...
	 * @param redirect
	 *            If true, makes an automatic redirect to the response
	 * @throws IOException
	...
	public boolean hasAccess(HttpServletResponse response, boolean redirect)
		...
		if (allowed) {
			if (m_request != null
					&& m_request.getAttribute(WikiTagBase.ATTR_CONTEXT) == null) {
		...
		if (!allowed && redirect) {
			...
			if (m_session.isAuthenticated()) {
				...
				m_session.addMessage(MessageFormat.format(
						rb.getString("security.error.noaccess.logged"),
						arguments));
		...
		if (template == null) {
			...
			if (page != null) {
				template = (String) page
						.getAttribute(WikiEngine.PROP_TEMPLATEDIR);
			}
	...
	protected static Command findCommand(WikiEngine engine,
			HttpServletRequest request, WikiPage page) {
		String defaultContext = PageCommand.VIEW.getRequestContext();
		Command command = engine.getCommandResolver().findCommand(request,
				defaultContext);
	...
	/**
	 * Protected method that updates the internally cached Command. Will always
	 * be called when the page name, request context, or variable changes.
	 * 
	...
	public ResourceBundle getBundle(String bundle)
		...
		ResourceBundle b = m_engine.getInternationalizationManager().getBundle(
				bundle, loc);
