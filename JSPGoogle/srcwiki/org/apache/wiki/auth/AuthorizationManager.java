 ...
 * {@link java.security.Permission} classes. There are two types of permissions:
 * </p>
 * <ul>
 * <li>{@link org.apache.wiki.auth.permissions.WikiPermission} - privileges that
 * apply to an entire wiki instance: <em>e.g.,</em> editing user profiles,
 * creating pages, creating groups</li>
 * <li>{@link org.apache.wiki.auth.permissions.PagePermission} - privileges that
 * apply to a single wiki page or range of pages: <em>e.g.,</em> reading,
 * editing, renaming
 * </ul>
 * <p>
 * Calling classes determine whether they are entitled to perform a particular
 * action by constructing the appropriate permission first, then passing it and
 * the current {@link org.apache.wiki.WikiSession} to the
 ...
 * </p>
 * <p>
 * For WikiPermissions, the decision criteria is relatively simple: the caller
 * either possesses the permission, as granted by the wiki security policy -- or
 * not.
 * </p>
 * <p>
 * For PagePermissions, the logic is exactly the same if the page being checked
 * does not have an access control list. However, if the page does have an ACL,
 * the authorization decision is made based the <em>union</em> of the
 * permissions granted in the ACL and in the security policy. In other words,
 * the user must be named in the ACL (or belong to a group or role that is named
 * in the ACL) <em>and</em> be granted (at least) the same permission in the
 * security policy. We do this to prevent a user from gaining more permissions
 * than they already have, based on the security policy.
 * </p>
 * <p>
 * See the {@link #checkPermission(WikiSession, Permission)} and
 * {@link #hasRoleOrPrincipal(WikiSession, Principal)} methods for more
...
public final class AuthorizationManager extends AbstractWikiProvider {
     ...
     * Permission is allowed for the Subject associated with a supplied
     * WikiSession. The access control algorithm works this way:
     * <ol>
     ...
     * <li>For all permissions, check to see if the Permission is allowed
     * according to the default security policy. If it isn't, deny the
     * permission and halt further processing.</li>
     ...
     * {@link org.apache.wiki.auth.acl.UnresolvedPrincipal}s (see below). Then
     * iterate through the Subject's Principal set and determine whether the
     * user (Subject) posesses any one of these specified Roles or Principals.
     ...
     * type <code>UnresolvedPrincipal</code>. This means that the last time the
     * ACL was read, the Principal (user, built-in Role, authorizer Role, or
    ...
    public final boolean checkPermission(WikiSession session,
        ...
        // Acl acl = (page == null) ? null : m_engine.getAclManager()
        // .getPermissions(page);
        Acl acl = (page == null) ? null : aManager.getPermissions(page);
        ...
        for (Principal aclPrincipal : aclPrincipals) {
            ...
            if (aclPrincipal instanceof UnresolvedPrincipal) {
                ...
                if (aclEntry != null
                        && !(aclPrincipal instanceof UnresolvedPrincipal)) {
     ...
     * a desired Role or GroupPrincipal. The algorithm simply checks to see if
     * the Subject possesses the Role or GroupPrincipal it in its Principal set.
     * Note that any user (anyonymous, asserted, authenticated) can possess a
     * built-in role. But a user <em>must</em> be authenticated to possess a
     * role other than one of the built-in ones. We do this to prevent privilege
     * escalation.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * Note that this method does <em>not</em> consult the external Authorizer
     * or GroupManager; it relies on the Principals that have been injected into
     * the user's Subject at login time, or after group
     * creation/modification/deletion.
     * </p>
     * 
     * @param session
     *            the current wiki session, which must be non-null. If null, the
     *            result of this method always returns <code>false</code>
     * @param principal
     *            the Principal (role or group principal) to look for, which
     *            must be non-<cor>null</code>. If <code>null</code>, the result
    ...
    public final boolean isUserInRole(WikiSession session, Principal principal) {
        ...
        if (session.isAuthenticated()
                && AuthenticationManager.isRolePrincipal(principal)) {
            return session.hasPrincipal(principal);
        }
        return false;
    }
    ...
    /**
     * Returns the current external {@link Authorizer} in use. This method is
     * guaranteed to return a properly-initialized Authorizer, unless it could
     * not be initialized. In that case, this method throws a
     * {@link org.apache.wiki.auth.WikiSecurityException}.
     * 
    ...
    public final Authorizer getAuthorizer() throws WikiSecurityException {
        ...
        throw new WikiSecurityException(
                "Authorizer did not initialize properly. Check the logs.");
    }
     ...
     * {@link #isUserInRole(WikiSession, Principal)} and return the result.</li>
     * <li>Otherwise, we're looking for a user Principal, so iterate through the
     * Principal set and see if any share the same name as the one we are
     * looking for.</li>
     * </ol>
     * <p>
     * <em>Note: if the Principal parameter is a user principal, the session
     * must be authenticated in order for the user to "possess it". Anonymous
     * or asserted sessions will never posseess a named user principal.</em>
     * </p>
     * 
     * @param session
     *            the current wiki session, which must be non-null. If null, the
     *            result of this method always returns <code>false</code>
     * @param principal
     *            the Principal (role, group, or user principal) to look for,
     *            which must be non-null. If null, the result of this method
     *            always returns <code>false</code>
     * @return <code>true</code> if the Subject supplied with the WikiContext
     *         posesses the Role, GroupPrincipal or desired user Principal,
     ...
     *            the wiki engine
     * @param properties
     *            the set of properties used to initialize the wiki engine
     * @throws WikiException
     *             if the AuthorizationManager cannot be initialized
     */
    ...
    /**
     * Returns <code>true</code> if JSPWiki's JAAS authorization system is used
     * for authorization in addition to container controls.
     * 
     * @return the result
     */
     ...
     * @throws WikiException
     */
    private final Authorizer getAuthorizerImplementation(Properties props)
            throws WikiException {
    ...
    private final Object locateImplementation(String clazz)
            throws WikiException {
        if (clazz != null) {
            try {
                ...
                throw new WikiException("Authorizer " + clazz
                        + " cannot be found", e);
                ...
                log.fatal(
                        "You are not allowed to access this authorizer class",
                        e);
        ...
        throw new NoRequiredPropertyException("Unable to find a "
                + PROP_AUTHORIZER + " entry in the properties.",
                PROP_AUTHORIZER);
    }

    /**
     * Checks to see if the local security policy allows a particular static
     * Permission. Do not use this method for normal permission checks; use
    ...
    protected boolean allowedByLocalPolicy(Principal[] principals,
        ...
        for (Principal principal : principals) {
            // Get ProtectionDomain for this Principal from cache, or create new
            // one
            ...
            ClassLoader cl = this.getClass().getClassLoader();
            CodeSource cs = new CodeSource(null, (Certificate[]) null);
            ProtectionDomain pd = new ProtectionDomain(cs, null, cl,
     ...
     * context's <code>codeBase</code> is effectively <em>this class</em>, not
     * that of the caller. Therefore, this method will work best when what
     ...
     * that simply calls
     * {@link java.security.AccessController#checkPermission(Permission)}.
     * 
     ...
     * This method is guaranteed to always return a Principal. The algorithm is
     * straightforward:
     * </p>
     * <ol>
     * <li>If the name matches one of the built-in
     * {@link org.apache.wiki.auth.authorize.Role} names, return that built-in
     * Role</li>
     * <li>If the name matches one supplied by the current
     * {@link org.apache.wiki.auth.Authorizer}, return that Role</li>
     * <li>If the name matches a group managed by the current
     * {@link org.apache.wiki.auth.authorize.GroupManager}, return that Group</li>
     * <li>Otherwise, assume that the name represents a user principal. Using
     * the current {@link org.apache.wiki.auth.user.UserDatabase}, find the
     * first user who matches the supplied name by calling
     * {@link org.apache.wiki.auth.user.UserDatabase#find(String)}.</li>
     * <li>Finally, if a user cannot be found, manufacture and return a generic
     * {@link org.apache.wiki.auth.acl.UnresolvedPrincipal}</li>
     * </ol>
     * 
     * @param name
    ...
    /**
     * Fires a WikiSecurityEvent of the provided type, user, and permission to
    ...
    protected final void fireEvent(int type, Principal user, Object permission) {
        if (WikiEventManager.isListening(this)) {
            WikiEventManager.fireEvent(this, new WikiSecurityEvent(this, type,
                    user, permission));
        }
    }
