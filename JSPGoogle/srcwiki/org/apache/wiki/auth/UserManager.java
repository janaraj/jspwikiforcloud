...
public final class UserManager extends AbstractWikiProvider {
    ...
    private final Map<WikiSession, UserProfile> m_profiles = WeakHashMapFactory
    ...
    public final void initializeProvider() {
        ...
        m_useJAAS = AuthenticationManager.SECURITY_JAAS.equals(m_properties
        ...
        JSONRPCManager.registerGlobalObject("users", new JSONUserModule(this),
    ...
    public final UserDatabase getUserDatabase() {
        ...
        if (cUser == null) {
            ...
            if (!m_useJAAS) {
                cUser = new CreateModuleManager(m_engine,
                        m_engine.getWikiProperties(), new DummyUserDatabase());
            } else {
                cUser = new CreateModuleManager(m_engine,
                        m_engine.getWikiProperties(), "userProvider");
            }
            ...
            // try
            // {
            // dbClassName = WikiEngine.getRequiredProperty(
            ...
            // {
            // log.error(
            // "You have not set the '"+PROP_DATABASE+"'. You need to do this if you want to enable user management by JSPWiki."
            // );
            // }
            // catch( ClassNotFoundException e )
            // {
            // log.error( "UserDatabase class " + dbClassName +
            // " cannot be found", e );
            // }
            ...
            // {
            // log.error(
            // "You are not allowed to access this user database class", e );
            // }
            // finally
            // {
            // if( m_database == null )
    ...
    /**
     * <p>
     * Retrieves the {@link org.apache.wiki.auth.user.UserProfile}for the user
     ...
     * its {@link org.apache.wiki.auth.user.UserProfile#isNew()} method will
    ...
    /**
     * <p>
     * Saves the {@link org.apache.wiki.auth.user.UserProfile}for the user in a
     * wiki session. This method verifies that a user profile to be saved
     ...
     *             {@link org.apache.wiki.auth.WikiSecurityException}; if if the
     *             user profile must be approved before it can be saved, it will
     *             be a
     ...
     */
    public final void setUserProfile(WikiSession session, UserProfile profile)
            throws DuplicateUserException, WikiException {
        // Verify user is allowed to save profile!
        Permission p = new WikiPermission(BeanHolder.getApplicationName(),
        ...
        if (!mgrA.checkPermission(session, p)) {
            throw new WikiSecurityException(
                    "You are not allowed to save wiki profiles.");
        }
        ...
        boolean nameChanged = (oldProfile == null || oldProfile.getFullname() == null) ? false
                : !(oldProfile.getFullname().equals(profile.getFullname()) && oldProfile
                        .getLoginName().equals(profile.getLoginName()));
        UserProfile otherProfile;
        try {
            otherProfile = getUserDatabase().findByLoginName(
                    profile.getLoginName());
            if (otherProfile != null && !otherProfile.equals(oldProfile)) {
                throw new DuplicateUserException("The login name '"
                        + profile.getLoginName() + "' is already taken.");
            }
        } catch (NoSuchPrincipalException e) {
        }
        try {
            otherProfile = getUserDatabase().findByFullName(
                    profile.getFullname());
            if (otherProfile != null && !otherProfile.equals(oldProfile)) {
                throw new DuplicateUserException("The full name '"
                        + profile.getFullname() + "' is already taken.");
            }
        } catch (NoSuchPrincipalException e) {
        }
        ...
        if (newProfile && oldProfile != null && oldProfile.isNew()) {
            ...
            Workflow workflow = builder.buildApprovalWorkflow(submitter,
                    SAVE_APPROVER, null, SAVE_DECISION_MESSAGE_KEY, facts,
                    completionTask, null);
            ...
            if (approvalRequired) {
                throw new DecisionRequiredException(
                        "This profile must be approved before it becomes active");
            }
            ...
            try {
                ...
                AuthenticationManager mgr = BeanHolder
                        .getAuthenticationManager();
                if (newProfile && !mgr.isContainerAuthenticated()) {
                    mgr.login(session, profile.getLoginName(),
                            profile.getPassword());
                }
        ...
        // For existing accounts, just save the profile
            ...
            if (nameChanged
                    && oldProfile != null
                    && !oldProfile.getLoginName()
                            .equals(profile.getLoginName())) {
                getUserDatabase().rename(oldProfile.getLoginName(),
                        profile.getLoginName());
            }
            ...
            if (nameChanged) {
                ...
                UserProfile[] profiles = new UserProfile[] { oldProfile,
                        profile };
                fireEvent(WikiSecurityEvent.PROFILE_NAME_CHANGED, session,
                        profiles);
            } else {
     ...
     * {@link org.apache.wiki.WikiSession#getLoginPrincipal()}. Otherwise, the
    ...
    public final UserProfile parseProfile(WikiContext context) {
        ...
        if (auth.isContainerAuthenticated()) {
            ...
            if (context.getWikiSession().isAuthenticated()) {
                loginName = context.getWikiSession().getLoginPrincipal()
                        .getName();
            }
        }
    ...
    public final void validateProfile(WikiContext context, UserProfile profile) {
        ...
        List<PageFilter> ls = BeanHolder.getFiltrListContainer()
                .getFilterList();
        ...
        validator.validateNotNull(profile.getLoginName(),
                rb.getString("security.user.loginname"));
        validator.validateNotNull(profile.getFullname(),
                rb.getString("security.user.fullname"));
        validator.validate(profile.getEmail(),
                rb.getString("security.user.email"), InputValidator.EMAIL);
        ...
        if (!auth.isContainerAuthenticated()) {
            ...
            if (password == null) {
                ...
                HttpServletRequest request = context.getHttpRequest();
                String password2 = (request == null) ? null : request
        ...
        try {
            ...
            if (otherProfile != null && !profile.equals(otherProfile)
                ...
                session.addMessage(SESSION_MESSAGES, MessageFormat.format(
                        rb.getString("security.error.illegalfullname"), args));
            }
    ...
    public static class SaveUserProfileTask extends Task {
        ...
        public Outcome execute() throws WikiException {
            ...
            UserProfile profile = (UserProfile) getWorkflow().getAttribute(
                    SAVED_PROFILE);
            ...
            if (profile.getEmail() != null) {
                try {
                    ...
                    String content = "Congratulations! Your new profile on "
                            + app
                            + " has been created. Your profile details are as follows: \n\n"
                            + "Login name: "
                            + profile.getLoginName()
                            + "\n"
                            + "Your name : "
    ...
    protected final void fireEvent(int type, WikiSession session, Object profile) {
        if (WikiEventManager.isListening(this)) {
            WikiEventManager.fireEvent(this, new WikiSecurityEvent(session,
                    type, profile));
        }
    }
