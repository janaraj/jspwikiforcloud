...
<%
    ...
    log.info("Page rename request for page '" + renameFrom
            + "' to new name '" + renameTo + "' from "
    ...
    try {
        if (renameTo.length() > 0) {
            PageRenamer pRename = BeanHolder.getPageRenamer();
            String renamedTo = pRename.renamePage(wikiContext,
                    renameFrom, renameTo, changeReferences);

            FixedQueue trail = (FixedQueue) session
                    .getAttribute(BreadcrumbsTag.BREADCRUMBTRAIL_KEY);
            if (trail != null) {
                trail.removeItem(renameFrom);
                session.setAttribute(
                        BreadcrumbsTag.BREADCRUMBTRAIL_KEY, trail);
            }

            log.info("Page successfully renamed to '" + renamedTo + "'");

            response.sendRedirect(wikiContext.getURL(WikiContext.VIEW,
                    renamedTo));
            return;
        }
        ...
        if (e.getMessage().equals(
            ...
            wikiSession.addMessage("rename", MessageFormat.format(
                    rb.getString("rename.exists"), args));
        } else {
            Object[] args = { e.toString() };
            wikiSession.addMessage(
                    "rename",
                    MessageFormat.format(
                            rb.getString("rename.unknownerror"), args));
        }
