...
<%!Log log = LogFactory.getLog("Edit.jsp");
...
<%
    ...
    if (!wikiSession.isAuthenticated() && wikiSession.isAnonymous()
            && author != null) {
        user = TextUtil
                .replaceEntities(findParam(pageContext, "author"));
    }
    ...
    //
    PageLock lock = pManager.lockPage(wikipage, user);

    if (lock != null) {
        session.setAttribute("lock-" + pagereq, lock);
    }

    String contentPage = BeanHolder.getTemplateManager().findJSP(
