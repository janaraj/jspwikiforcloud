...
public class WikiGaePageProvider extends AbstractWikiProvider implements
    ...
    private class EPutPageTextDirect extends ECommand {

        private final WikiPage page;
        private final String text;

        EPutPageTextDirect(WikiPage page, String text) {
            super(true);
            this.page = page;
            this.text = text;
        }
        ...
        protected void runCommand(EntityManager eF) {
            ...
            RetResult ret = getPageInfoW(eF, page.getName(), page.getVersion());
            if (ret.ent == null) {
                ret.ent = new WikiPageEnt();
            }
            ret.ent.setVersion(page.getVersion());
            ...
            ret.ent.setName(page.getName());
            ret.ent.setChangeBy(page.getAuthor());
            log.debug("Put page direct, store text, size =" + text.length());
            log.debug(text);
            ret.ent.setContent(text);
            ret.ent.setChangeNote(page.getAttribute(WikiPage.CHANGENOTE));
            eF.persist(ret.ent);
    ...
    public void putPageText(WikiPage page, String text)
        ...
        ECommand e;
        if (page.isPutdirect()) {
            e = new EPutPageTextDirect(page, text);
        } else {
            e = new EPutPageText(page, text);
        }
        e.runCommand();
