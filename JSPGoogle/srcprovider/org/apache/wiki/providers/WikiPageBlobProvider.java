...
import org.apache.wiki.providers.PageUtil.WikiVersion;
...
public class WikiPageBlobProvider extends AbstractWikiProvider implements
	...
	private static final String cacheName = "JSPWIKI-PAGE";

	private Cache cacheC;

	private String getCacheName(String pageName, int version) {
		return pageName + "-" + version;
	}

	private String getCacheExist(String pageName) {
		return pageName + "-EXISTS";
	}

	private String getCache(String pageName, int version) {
		if (cacheC == null)
			return null;
		return (String) cacheC.get(getCacheName(pageName, version));
	}

	private void putCache(String pageName, int version, String content) {
		if (cacheC == null)
			return;
		cacheC.put(getCacheName(pageName, version), content);
		cacheC.put(getCacheExist(pageName), "X");
	}

	private String getCacheNameInfo(String pageName, int version) {
		return getCacheName(pageName, version) + "-WIKI-VERSION";
	}

	private void putCacheInfo(String pageName, int version,
			PageUtil.WikiVersion v) {
		if (cacheC == null)
			return;
		cacheC.put(getCacheNameInfo(pageName, version), v);
	}

	private PageUtil.WikiVersion getCacheInfo(String pageName, int version) {
		if (cacheC == null)
			return null;
		return (WikiVersion) cacheC.get(getCacheNameInfo(pageName, version));
	}

	private boolean pageCacheExists(String pageName) {
		if (cacheC == null)
			return false;
		return cacheC.containsKey(getCacheExist(pageName));
	}
	...
	public WikiPageBlobProvider() {

		try {
			CacheFactory cacheFactory = CacheManager.getInstance()
					.getCacheFactory();
			cacheC = cacheFactory.createCache(Collections.emptyMap());
	...
	public WikiPage getPageInfo(String page, int version)
			throws ProviderException {
		PageUtil.WikiVersion v = getCacheInfo(page, version);
		if (v != null) {
			WikiPage ww = new WikiPage(page);
			try {
				PageUtil.toWikiPage(ww, v);
				return ww;
		...
		pa.runCommand();
		putCacheInfo(page, version, pa.v);
		return pa.pa;
	}
	...
	public String getPageText(String page, int version)
			throws ProviderException {
		String text = getCache(page, version);
		if (text != null)
			return text;

		PageText pa = new PageText(page, version);
		pa.runCommand();
		putCache(page, version, pa.content);
		return pa.content;
	}
